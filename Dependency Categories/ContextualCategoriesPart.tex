...
and such that for all $i$, $1 \leq i \leq n$, there exists $j_i$, $1 \leq j_i \leq m$
such that $A_i \leq B_{j_i}$
and there exists $j'_i$, $1 \leq j'_i \leq m'$, such that $A_i \leq B'_{j'_i}$ and such that
\begin{equation}
\label {ddtwobjai}
b_{j_i} \circ \pc{B_{j_i}}{A_i} = \pc{Y}{W} \circ a_i
\end{equation}
and 
\begin{equation}
\label {ddtwobpjpai}
...
\noindent
Condition (\ref{ddtwobjai}) implies that, for each $i$, $1 \leq i \leq n$,
\begin{center}
\begin{equation}
%\label{pbdrilldowncospan}
\begin{array}{cp{0.1cm}cp{0.1cm} c}
\Rnode{Bji}{B_{j_i}}& &                & & \Rnode{Ai}{A_i} \\ [0.4cm]
            & &\Rnode{AiBottom}{A_i}
\end{array}
\end{equation}
\nccdar{Bji}{AiBottom}
...
\begin{array}{cp{0.1cm}cp{0.1cm} c}
            & &\Rnode{Z}{Z}                  \\ [0.4cm]
\Rnode{Bji}{B_{j_i}}& &                & & \Rnode{Ai}{A_i} 
\end{array}
\end{equation}
\nccdar{Z}{Bji}
\blabel{f \circ b_{j_i}}
\nccdar{Z}{Ai}
...
g \circ \pc{Y}{W} \circ a_i                      
     &= g \circ b_{j_i} \circ \pc{B_{j_i}}{A_i}  && \mbox{by (\ref{ddtwobjai})}  \\
     &= g_{j_i} \circ \pc{B_{j_i}}{A_i}          && \mbox{by (\ref{ddtwofactorgj})}  \\
		&=g''_i                                      && \mbox{by (\ref{ddtwogjigppi})}  \\
\end{align*}
The proof for the rhs is similar and so (\ref{ddtwoggpa}) is proved. 
