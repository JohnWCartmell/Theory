...
\end{lemma}
\begin{proof}
Inductive step. Assume  that the lemma holds of all objects with a height less than the height of $Z$. Suppose $Z=\crossx{Y}{Y'}{X}$. First note that in this situation 
$\pbase{Z}=\pbase{Y}$. We show by cases that required limit cone exists.
\begin{enumerate}[(i)]
\item{Y, Y' and X are all prime objects}
Because $X$ is prime it follows that $X \leq  \pbase{Y}$. 
