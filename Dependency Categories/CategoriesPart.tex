...
\usepackage[table]{xcolor} % to gray colour rows of tables
...
\begin{lemma}
\llabel{permutationnormalisation}
...
Suppose $\sim_A$ is the equivalence relation that partitions the set $\set{1,...5}$ into
two equivalence classes $\set{1,2}$ and $\set{3,4,5}$ and that $\sim_B$  is the equivalence relation that partitions $\set{1,...5}$ into $\set{1,2,3}$ and $\set{4,5}$.
If $\sigma = \bigl(\begin{smallmatrix}
    1 & 2 & 3 & 4 & 5 \\
    5 & 4 & 2 & 3 & 1
...
\end{tabular}
\end{tabular}
\ncline[nodesep=3pt]{|->}{R1}{L5}
\ncline[nodesep=3pt]{|->}{R2}{L4}
\ncline[nodesep=3pt]{|->}{R3}{L2}
\ncline[nodesep=3pt]{|->}{R4}{L3}
\ncline[nodesep=3pt]{|->}{R5}{L1}
then it meets the specified condition. 
In this case 
 $\sigmabar = \bigl(\begin{smallmatrix}
    1 & 2 & 3 & 4 & 5 \\
    4 & 5 & 1 & 2 & 3
  \end{smallmatrix}\bigr) $ as illustrated here
\begin{tabular}{ c p{2cm} c }
\begin{tabular}{ c }
  $\sim_B$ \\ \hline
	         \\[-0.3cm]
...
\end{tabular}.
\end{tabular}
\ncline[nodesep=3pt]{|->}{R1}{L4}
\ncline[nodesep=3pt]{|->}{R2}{L5}
\ncline[nodesep=3pt]{|->}{R3}{L1}
\ncline[nodesep=3pt]{|->}{R4}{L2}
\ncline[nodesep=3pt]{|->}{R5}{L3}
\end{example}
...
such that \foreachi, $A_{\sigma(i)}=B_i \mbox{ and }  c_A(\sigma(i)) = c_B(i)$. \commentary{This corrected.} In the absence of unique factorisation this definition needs to be revised. For this 
...
\commentary{\highlight{Big doubt here}: Is it well-defined???}
