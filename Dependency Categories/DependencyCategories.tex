...
\cite{Cartmell86B}. Dependency categories are a variation on the notion of contextual category (\cite{Cartmell78}, \cite{Cartmell86})--  the category of dependency categories, as defined here, is equivalent to the category of contextual categories.
...
\noindent The objects of contextual categories can be thought of as contexts or as 
types that vary (also known as dependent types) but equally they can be thought of as entity types in the sense used in entity modelling. In the entity modelling domain the
morphisms of a contextual category correspond with the many-one binary relationships that are the staple fare of entity modelling. There  is a distinguished subset of morphisms in a contextual category  that collectively form a hierarchical backbone. This contrasts
with the situation in a dependency category where the distinguished morphisms do not form a hierarchy but a wide acyclic subcategory. In either case the distinguished morphisms correspond to distinguished many-one binary relationships in a certain style 
of entity modelling where they are called composition relationships (\cite{ECMA-149}, \cite{Munck1988}, \cite{entitymodelling}) .

\noindent 
Previously the inflential paper by Chen \cite{Chen1976} introduced the idea of entities being dependent on binary relationships 
