...
A matrix has rows, columns, elements and, for each element, numeric content. 
In the style of modelling introduced in \cite{entitymodelling} the structure of a numeric matrix can be represented 
by entity types and binary relationships as shown in figure \ref{sparseMatrix}.
...
\raisebox{-4.8cm}{\input{sparseMatrix}} & &
...
\noindent
Similarly in the phases:
\begin{equation}
\mbox{\textit{I program the computer.}}
\end{equation}
\noindent and
\begin{equation}
\mbox{\textit{I write a computer program.}}
\end{equation}
the word 'program' occurs on the one as a `verb' and in the other as a `noun'. 
This illustrates that a word in and by itself cannot be said to 
be a `noun', `verb', 'adjective' or such part of speech like lest it be appearing in some grammatical sentence so `noun' and `verb`
and other parts of speech as types of thing are dependent on sentence-like types of thing.
In fact, in turn, the type of sentence-like types of thing depends on the language that is being spoken or written,English, French or
whatever. \\

\noindent
Types such as these may be described by entity modelling or more flexibly by a mathematical notation. Types
that vary we say are dependent types.  \\
...
Concepts like `angle', `edge', `boundary', `bounding line' as we learn the concepts,
we learn that do not stand alone; they are dependent concepts as are  face of cube,  endpoint of line,   junction between lines, citizen (of a country), tangent (to a curve), atom of a molecule,  nucleus of a cell, character of play. \\

\subsection{Quantities that Vary}
...
We also write $f: A \rightarrow B$ for such a function and consider there to be a category of types and functional dependencies and represent in the diagrams of catgeory theory.

\noindent
A different diagrammatic notation is available from the sphere of Information Systems Development,  entity model diagrams in the style of Barker, Ellis, Martin, Schlaer-Mellor and others (see \cite{Rock-Evans1989}), focus  on binary relationships between types of entity; 
...
or as a function with whose domain is the cartesian product of $A1$ and $A2$ which generally would be represented as $A_1 \times A_2$. 


\subsection{Types that Vary - Dependent Types}
 Consider that types are a kind of abstract quantity and we are led to the possibility that types
...
notation used for quantities that vary can equally be used for types that vary i.e. dependent types. 
...
\noindent We see now that dependencies compose in different ways in the two cases and that therefore the category of dependencies is significant to characterising the type dependency relationships.
