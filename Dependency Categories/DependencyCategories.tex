...
coincident  and there does no exist cospan 
\begin{center}
\begin{equation}
\begin{array}{cp{0.1cm}cp{0.1cm} c}
\Rnode{c1}{C_1}& &                & & \Rnode{c2}{C_2} \\ [0.4cm]
...
where $\chi_{i,j}$ is the characterising family for the pair $f_i,f_j$ and 
...
\begin{definition}
If \cat{C} is a well-founded category then a family of morphisms  $f_i: C \morph B_i$ is said to
be a basis for object C iff for every morphism $f: C \morph X$ in \cat{C} either 
$X$ is a limit of its characterising diagram or else $f$ factors through one of $f_i$ for some $i$ i.e there exists $g:B_i \morph X$, for some $i$, such that $f_i \circ g = f$.
\end{definition}

\section{Cones and Tight Cones}
Recall the definition of cone $\tuple{ N,\psi}$ to a diagram $F:J \morph C$ of a category $C$ as an object $N$ of $C$ and a family of morphisms $\psi_j$ indexed by objects $j$ of $J$ such that for all $f: j \morph j'$ in $J$, $\psi_j \circ F(f) = \psi_{j'}$. Define a cone $\tuple{ N,\psi}$to be \textit{tight} iff for all pairs $j,j'$ of objects of shape category $J$, every minimal coincident cospan of the span:
...
the mediating morphism $h: N \morph B_1 \underset{D}{\Join} B_2$ is a d-moprphism iff the cone $\tuple{N,\psi}$ is tight. [Rephrase this using characteristic diagram?] 
...
\noindent For any 
cone $\tuple{N,\psi}$ to diagram $D$  [REPHRASE!!]
the mediating morphism $h: N \morph \underset{x_1=x_2}{\sigma}B$ is a d-moprphism iff the cone $\tuple{N,\psi}$ is tight. [Then rephrase again this using characteristic diagram?] 
...
the mediating morphism $h: N \morph B_1 \underset{D}{\Join} B_2$ is a d-moprphism iff the 
cone $\tuple{N,\psi}$ is tight. [Rephrase this using characteristic diagram?] 
