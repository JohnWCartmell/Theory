...
such that $f=d_1 \circ d_2$ then we will say f is a simple distinguished morphism (or s-morphism). We will write  $f:B \smorph A$ in \cat{C}
to mean that 
...
\begin{enumerate} [A.,leftmargin=0.5cm]
	...
	\item Within the category of distinguished morphsims every morphisms is an epimorphism.
...
\item	
an object  $ \fibrex{B_1}{B_2}{D}$ of \cat{C} and s-morphisms\footnote{Correction: $p_1$ and $p_2$ may not be s-morphisms - check and change and document as footnote.} 
