...
If $F$ is a mono-source with domain $a$ 
\end{lemma}
\begin{proof}
\end{proof}
...
transformed model $\logtophys(\genericmodel)$  is in Boyce-Codd Normal Form.\commentary{Either add condition that purely logical 
or, preferable, extend definition of subsumes also get rid of paths of length 1.}
\end{theorem}
\begin{proof}
...
which is equivalent to a simple path of $\genericmodel$ of length $\geq 2$ which contradicts  the 
 initial assumption that the model $\genericmodel$ is purely logical\commentary{TBD} and so completes the proof.
\end{proof}
