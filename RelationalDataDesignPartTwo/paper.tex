...
\begin{definition}
We say that a referential inclusion dependency
$\incd{a}{f_1,...f_n}{b}{q_1,...q_n}$  in an ER model $\genericmodel$ 
 is explicitly represented in the model
iff the path $\tuple{f_1,...f_n,I_b}$ is explicitly represented
i.e. iff there is a simple path $p:a \morph b$ in $\genericmodel$
such that in all instances $E$ of $\genericmodel$, $E_{\fntuple} \circ E_{I_b}=E_p$.
\end{definition}
\begin{lemma}
...
$\incd{a}{f_1,...f_n}{b}{q_1,...q_n}$  in an ER model $\genericmodel$ 
 is explicitly represented in the model 
iff there is a path $p:a \morph b$ in $\genericmodel$ such that \foreachi, $p \circ q_i \simeq f_i$.
\end{lemma}
\begin{proof}
TBD.
\end{proof}
...
$d$, $\qntuple$, such that for each $q_i:d \rightarrow b_i$ and a simple
...
\ref{simplerepresentationlemma}, sufficient that all simple referential inclusion dependencies be explicitly represented.} are explicitly represented,
}
\item{
for each intransitive functional dependencies $\{x_1,...x_n\} \morph y$ in model \genericmodel\
either in every instance $E$, $E_{\langle x_1,..x_n \rangle}$ is injective and total
...
Because $\incd{a}{x_1,...x_n}{d}{q_1,...q_n}$, because \genericmodel\ is well formed and from
clause (ii) of the definition of well-formedness it follows that
there  is a simple path $p:a \rightarrow d$ such
that \foreachi, $p \circ q_i \simeq x_i$ and therefore
in all instances $E$,
$$
E_{\langle x_1,..x_n \rangle} \circ inv_{E_{\qntuple}} = E_p
$$.
From xxx it follows that in all instances $E$ that 
\begin{align*}
E_y &= E_{\xntuple} \circ E_{I_{\qntuple}} \circ E_z\\
    &= E_p \circ E_z  \\
		&= E_{p \circ z} 
\end{align*}
Therefore $y\simeq p \circ z$ as required.
\end{proof}
