...
\maketitle
\begin{center}
DRAFT \marginpar{\footnotesize Original Dated 2016 }
      \marginpar{\footnotesize Review started 21 Feb 2019. }
      \marginpar{\footnotesize Addition of variant definitions, notes and  corrections 25/29 April 2019.}
			\marginpar{\footnotesize Major change to terminology to support fix to definition of well-                               formulated including specification of intransitive functional 
...
\item{\marginpar{\footnotesize Review this clause, modified 5 May 2019.}
...
then path $y$ is said to be $functional dependent$ on the set of paths $\{x_1,...x_n\}$ iff
 in each instance $E$ there exists a unique partial \marginpar{\footnotesize why unique?}
...
\subsection{Corrections required to the $\chiZero$ Transform - Definition of $\chiOne$}
\subsubsection*{Requirements - Shortcomings of $\chiOne$}
...
\subsubsection*{Removing the first level of redundancy from $\chiZero$ -- Definition $\chiOne$}
...
\marginpar{\footnotesize previously : for some j, $j > 1$,
...
\noindent
We define $\chiOneFKs{a}$ to be the subset of $\chenFKattrs{a}$ consisting of those paths for which there are no paths that subsume them. \\
 
\subsection{Corrections required of the $\chiOne$ Transform -- Definition of $\logtophys$}

\subsubsection*{Requirement}
Example 3 is of the model given in figure \ref{datatablegraph} and shows equivalent paths.
In this case it is not that a column has to be excluded it is more so that it must not be included twice over. Mathematically we can represent as an equivalence class.

\subsubsection*{Correction to definition - Definition of Chi}
\noindent We define the Chi transformed model $\chigenericmodel$ by:
$$
\chiAttrs{a} = \attrs{a} \cup \chenFKclasses{a}
$$
\noindent where $\chenFKclasses{a}$ is the set of equivalence classes of paths in $\chiOneFKs{a}$
...
\marginpar{\footnotesize previously : and an identifying family of simple paths}
...
which implies that $\langle y_1,..y_m \rangle$ is excluded from $\chiOneFKs{a}$ and thus that 
...
\marginpar{\footnotesize Try and get rid of this in favour of fds being `elementary'.}
