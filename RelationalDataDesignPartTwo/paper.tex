...
\end{center}
and
\commentary {Removed that $z_1$ is not identifying}         
...
that for each $i$, $x_i : a \rightarrow b_i$, and such that $y: a \rightarrow c$   
and if there is an intransitive functional dependency  $\{x_1,...x_n\} \morph y$
then
either in every instance $E$, $E_{\langle x_1,..x_n \rangle}$ is injective and total or else
...
Note that $z$ cannot be equivalent to a primary key attribute for if it were then by lemma xxxx 
