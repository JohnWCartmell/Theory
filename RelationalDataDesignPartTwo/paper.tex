...
\begin{lemma}
\label{identifyingpathslemma}
If
\setlength{\arraycolsep}{.2cm}

$
\begin{array}{cp{2cm}c c}
              &         & & \Rnode{v1}{v}  \\ [0.5cm]
              &         & & \Rnode{v2}{v}  \\ [0.2cm]
 \Rnode{b}{b} &         &  \vdots &                \\ [0.3cm]
              &         & & \Rnode{vn}{v}  
\end{array}
$
\simplepathb{$q_1$}{b}{v1}
\simplepathb{$q_2$}{b}{v2}
\simplepathb{$q_n$}{b}{vn}
is an identifying tuple of simple paths wrt $b$
and if $z=\tuple{z_1,...z_l}$ is a simple path,
\setlength{\arraycolsep}{.2cm}
$
\begin{array}{cp{2.2cm}c}
\Rnode{b}{z:b}  & &\Rnode{v}{v}
\end{array}
$
\simplepatha{}{b}{v},
  in which each $z_j$ is identifying
then $z \simeq q_i$, for some $i$,$1 \leq i \leq n$.
\end{lemma}
\begin{proof}
...
\marginpar{\footnotesize This isn't correct if defs revised to allow foreign keys. Need understand this better. }
