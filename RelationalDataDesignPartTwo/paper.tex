...
but departed from the Chen notation and terminology and eliminated n-ary relationaships from the modelling
...
In this way, every entity is defined to have identifying features and two entities are necessarily identical if their identifying features are identical (so implementing the identity of indiscernibles). 
...
style of entity relationship modelling primary keys come about indirectly via specification of these identifying relationships and attributes. In the example shown in figure \ref{chenfragment} indication of the identifying features of each of the four entity types $p$, $e$, $w$ and $dep$ has been given indirectly by marking certain attributes and relationships as \textit{identifying} by way of a bar across the corresponding arrow -- an adaption of the bar notation used in Barker's book (but in the Barker-Ellis notation is different roughly speaking the arrow head is swapped for a crowsfoot at the other end of the line). If identifying relationships are used instead
...
some notional idea of set of instances that we envisage for it. A second approach is to extend the structure of a directed
graph by adding an equivalence relation of paths -- mathematicians will recognise this 
...
instance $E'$ of $\genericmodel'$ such that for edges $e$ in \genericmodel,
...
\item for each entity type $a$ if $P_a$ is the set of primary key paths of $a$ in \genericmodel\
and $P'_a$ is the set of primary key paths in $\genericmodel'$ then $P_a \simeq P'_a$ in $\genericmodel'$. 
\end{enumerate}
\end{definition}

\begin{definition}
Say that two models are \term{equivalent} iff there is a third model that they are both extensions.
\commentary{May need replace this by a symetric transitive closure.}
\end{definition}
...
\begin{definition}
A model \genericmodel\ satisfies the minimality condition 
iff for all entity type $a$, for all path mono-sources $j  \in \bar{I}$
if $j'$ is a proper subsets of $j$ which is a  monosource in \genericmodel then $j \simeq j'$. 
\end{definition}
...
This is an example of an ER model that fails the minimality condition.
because
...
mono-source and the path $\tuple{I,mNo}$ is not equivalent to either of these two paths.
...
\begin{definition}
If $P$ and $Q$ is a set of paths of model \genericmodel\ then say that set of paths $P$ 
\term{is representative of} $Q$ 
iff $P \simeq Q$ and no proper subset of $P$ is equivalent to $Q$.
\end{definition}

\begin{lemma}
A model \genericmodel\ satisfies the minimality condition iff whenever $P$ is representative
of the set $Q$  of key paths of \genericmodel\ then no proper subset of $P$ is a mono-source.
\end{lemma}
\begin{proof}
\end{proof}
...
\subsubsection{Minimality Condition of Factored Model}
...
\end{lemma}
\begin{proof}
Uses lemma \ref{identifyingfactorminimal} but isn't trivial -- needs writing out in full.
\end{proof}
