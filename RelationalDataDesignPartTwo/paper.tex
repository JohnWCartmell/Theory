...
edges have destination $\veee$. \commentary{Need do better than this}.
\end{definition}
...
\begin{definition}
The set $I$ of sets of sets of identifying edges of a model \genericmodel\  is \term{fully-factored} iff 
there does not exist an entity type $a$ $H \in I$ such that for some subset $\set{h_1,...h_n} \subset I_a$
...
Show that these two  definitions are equivalent.\commentary{Add a third form using composition notation used in lemmas.}

Show that if $I' = (I \backslash H) \cup \set{H_0 \cup \set{f}}$ then $\bar{I'} = \bar{I} \cup \set{H_0 \cup \set{f}}$.
\commentary{By lemma 
