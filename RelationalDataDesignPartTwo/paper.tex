...
In the example shown in figure \ref{chenfragment} indication of the primary keys of each of the four entity types $p$, $e$, $w$ and $dep$ has been given indirectly by marking certain attributes and relationships as \textit{identifying} by way of a bar across the corresponding arrow
in the diagram. This borrows from the bar notation used in Barker's book (but of course I use arrows for the edges representing relationship which the Barker-Ellis notation in which the arrow head is swapped for a crowsfoot at the other end of the line). If identifying relationships are used instead
...
which are specified in an entity model and the primary keys are derived from them. It is because there 
...
\section{Background}
\noindent 
In relational data modelling, every row of data is distinguishable from others by the values of a specific set of columns said to 
comprise the primary key whereas in the earliest presentation of entity relationship modelling \commentary{Previously: whereas in logical entity relationship (ER) modelling each entity is distinguishable from the values of a specific set of attributes taken in combination with a specific set of relationships with other entities\footnote{Whichever methodology is followed the goal is to achieve for the database instances the logical principal of identity of indiscernibles.}}
Chen (\ref{Chen76}) depicts specifications of certain types of entity to include specification of
 their primary keys in a syntax that enables the primary key to be a combination of an attribute in conjunction with a relationship with another entity\footnote{Other types of entity have a combination of relationships to other entities as their primary keys. These are his 'relationships' but they are not what later authors and ourselves here mean by this term.}. This essential feature 
is retained as the entity relationship model is re-imagined by other authors and it is represented fully in the Barker book where identifying attributes and relationships are distinguished from others on entity model diagrams. 
We simplify the Barker notation for theoretical purposes so that attributes and relationships are shown as edges of a directed graph and identifying features are shown by a bar\footnote{A Barker bar, we might say.} across the corresponding edge of the graph. 
In this style a fragment of an example from Chen is shown in figure (\ref{chenexample}). This diagram and others that follow are given to illustrate the theoretical possibilities for the Barker notation and notations of a similar ilk  -- we need emphasise that we do not offer the directed graph notation for practical use but use it to illustrate theory. 
...
\commentary{Defn of model moved earlier}.
...
\term{identifying tuple with respect to an entity type $a$}\commentary{an alternate defn. now given earlier}
