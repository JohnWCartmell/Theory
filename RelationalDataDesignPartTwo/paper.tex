...
style of entity relationship modelling primary keys come about indirectly via specification of these identifying relationships and attributes. In the example shown in figure \ref{chenfragment} indication of the primary keys of each of the four entity types $p$, $e$, $w$ and $dep$ has been given indirectly by marking certain attributes and relationships as \textit{identifying} by way of a bar across the corresponding arrow
...
Before we commence the theory we give one further example. 
\commentary{Was to give the example
from black notebook 7th May 2019 as
a counter to the inference of primary keys from identifying relationships 
but now realise it isn't -- provided the model is extended to be well formulated.
See black notebook 29 May 2019. Handbrake turn.}
It is an example that puts paid to the idea that 
identifying relationships and equivalence of paths can replace primary keys in the definition of an ER model. 
The example is shown in figure \ref{} 

and identifying relationships and attributes and shown. In this example there is
a path equivalence of $\tuple{}$ and $\tuple{}$.
