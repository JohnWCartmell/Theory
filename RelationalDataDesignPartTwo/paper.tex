...
If $A$ is a set, if $I$ is a jointly injective set of functions with domain $A$ and if $f \in I$, $f: A \morph B$ is a function,
and is $J$ is a jointly injective set of functions with domain $B$ then the set of functions
$(I \backslash f) \cup \set{f \circ j | j \in J}$ is jointly injective.
\end{lemma}
\begin{proof}
...
\commentary{If identifying sets are composed then the composed set is jointly monomorphic.}
If $F$ is a mono-source with domain $a$,if $f \in F$, $f: a \morph b$, and if $G$ is a mono-source with domain $b$
then $(F \backslash \set{f}) \cup \set{f \circ g |  g \in G}$ is a mono-source with domain $a$.
\end{lemma}
\begin{proof}
We need show that the family of functions $E_{e ) e \in H}$ is jointly injective for any instance $E$ of \genericmodel\
where $H$ is the set of paths $(F \backslash \set{f}) \cup \set{f \circ g |  g \in G}$.

For this use  lemma \ref{jointlyinjectivecomposition}.
\end{proof}
