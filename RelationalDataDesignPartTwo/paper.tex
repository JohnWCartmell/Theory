...
KEY && \\
\hline
o & order & Identified by a order number ($oN$) attribute. \\
d & delivery & Identified by a combination of relationship $Q$ to an order that it part fulfills and a
delivery number  attribute ($dN$). \\
w & warehouse & Identified by a warehouse number attribute ($wN$). \\
si & stock item & Identified by a combination of  item number iN and relationship $R_1$ to the warehouse in which it is stocked.\\
di & delivery item & Identified by relationship $S_0$ to the delivery  it is part of and relationship $R_0$ to the item it constitutes the delivery of. \\
\end{tabular} 
\end{tabular}
\end{center}
\caption{Example of transitive dependence - In this model the path $\tuple{R_0,R_1}$ is equivalent to path $\tuple{S_0,S_1}$. The path $\tuple{R_0,sC}$ depends transitively on the paths
$\tuple{S_0,Q,oN}$, $\tuple{S_0,dN}$ and $\tuple{R_0,iN}$. The dependence is transitive
because there is a path $P=\tuple{S_0,S_1}$ and $P$ depends on $\tuple{S_0,Q,oN}$ and $\tuple{S_0,dN}$
and $\tuple{R_0,sC}$ depends on $P$ and $\tuple{R_0,iN}$. }
\label{deliveryexample}
\end{figure}
