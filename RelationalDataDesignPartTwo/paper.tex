...
\begin{enumerate} [(i)]
\item{
for each entity type $a$, there is no proper subset $I$ of any set of identifying paths leaving $a$ that  is jointly monomorphic,
}
\item{ 
...
if there is an non trivial intransitive functional dependency
\sfd{x_1,...x_n}{y}
...
$\sfd{\phys{x}_1,...\phys{x}_n}{\phys{y}}$ is a non-trivial intransitive functional dependency
...
By lemma \ref{fdcarrythroughlemma} it follows that in the model $\genericmodel$ therefore, for each $i$, $1 \leq i \leq n$, 
...
\iffalse {%Maybe move this text later
...
In the latter case the fucntional dependency implies that in every instance instance $E$ of $\genericmodel$ there is a unique 
function $E_f$ such that 
\begin{equation}
\label{givenfd}
E_{<x_1,...x_n>} \circ E_f = E_{y}
\end{equation}

From the definition of a functional depedency being represented it follows that
there
...
\end{equation*}
\noindent and thus from (\ref{givenfd}) it follows  that:
\begin{equation}
...
FRom (ref{representationByIdentifyingRelationships}) it follows that there is an inclusion dependency $\incd{a}{x_1,...x_n}{d}{q_1,...q_n}$ in $\genericmodel$ and from the assumtion that  $\genericmodel$ is well-formulated it follows
