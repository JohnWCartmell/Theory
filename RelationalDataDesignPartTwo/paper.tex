...
\simplepathb{$y$}{a}{c}
\end{center}
\noindent then if in each instance $E$ there exists a unique 
function $f_E: E_{b_1} \times E_{b_n} \rightarrow E_c$ 

\noindent such that 
domain of $f_E \subseteq img(E_{\langle x_1,... x_n \rangle})$ 
and  for each $i$, $1 \leq i \leq n$, $E_{\langle x_1,..x_n \rangle} \circ f_E = E_y$ 
...
by an entity type $d$, and an identifying tuple of simple paths with respect to $d$,
 $q_1,...q_n$, $z$ such that  $\langle x_1,..x_n \rangle$ references $d$ with respect to
$q_1,...q_n$ 
