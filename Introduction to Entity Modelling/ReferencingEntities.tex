...
\subsection{Simple Path Arrangements of Identifying Relationships}\commentary{Russian Doll references}
...
and \textit{production}s need be referenced using four referentials (section \ref{exampleReferencingProductions})
...
First though, that only five referentials are needed can be seen from this reference to a dramatic role:
\begin{equation}
...
and make something clear that couldn't be inferred from the diagram alone,  that
\begin{equation}
...
\end{equation}
In this we make clear that in a reference to a dramatic role, the referenced character and the referenced production will be in relationship with the very same play
and that in referencing a dramatic role, the referencing of the character and the referencing of
the production will not be independent
--- each of these two references --- that to a character and that to a production ---
 must include a subordinate reference to the very same play;  these two, required subordinate references collapse to a single reference --- the reference to a play need not be duplicated. 
Finally, as we have seen, a reference to a play consist of two referentials; so
by counting this this subordinate reference only once, we save two referentials, which explains why only five  are required, not seven.
...
\end{oldtt}
\begin{oldtt}
We have  already seen in other examples that nested references \commentary{I don't understand this --- is it plain wrong?-- I believe so}
can have other references nested within them;
now we find that these other, subordinate, references can be shared so they are not truly nested.  We find this in 
\textit{the role of Touchstone, Spring '75 at the Oxford Playhouse} which is a reference that contains two nested references; each of these has a subordinate nested reference and this is common to the both of them i.e. shared between them. Necessarily, this subordinate 'nested' reference isn't 
physically nested within the both of them --- the nested box analogy breaks down.
\end{oldtt}
\mynote
Now a crucial observation, the double nesting of references comes about because of this arrangement of relationships 
\begin{equation}
...
The ``in the same play'' injunction is necessary \commentary{\highlight{work on} smoothing above diagram with this}
...
\highlight{turn it around: the injunction make sense because of this arrangement of relationships ... because there are two ways around the diagram from dramatic role to play}
