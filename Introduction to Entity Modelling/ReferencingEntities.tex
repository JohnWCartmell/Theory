...
\label{ReferencingEntities}\commentary{\highlight{renumber all the examples} so that run 1 thru 7}

We now explore how the arrangement of identifying features of a type of entity guides us in referencing entities of that type, and how this, in turn, influences the ways we convey, communicate, and store instances of relationships.
There is more to this than initially meets the eye.
Through examples, we will see that comparable paths that are equivalent or near-equivalent (as described in Section~\ref{comparisonOfPaths}) affect how we phrase references.
...
In such a situation, a reference to an entity of type $A$ includes, as a subreference, a reference to an entity of type $B$.
...
\commentary{I am beginning to think this moves later --- right to the end of chapter 9}
