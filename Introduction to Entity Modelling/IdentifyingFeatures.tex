...
so that the airport terminal relationship is represented, \commentary{consider adding code as primary key to airport}\commentary{change to  one or more terminals per airport}
...
Assertion (\ref{JupiterIoPernickityOne}) is, in fact, an example of a wider standard phraseology, one  for conveying relationship instances;
...
\begin{newtt}THIS IS WHERE MY REVIEW IS UP TO 12:45 19 September.\end{newtt}
\mynote\commentary{reword this sentence}
\begin{oldtt}
Paths of identifying relationships arise whenever 
entities of a type are wholy or partly identified by relationships with other types which in turn are, in part at least, identified by relationships.
\end{oldtt}
