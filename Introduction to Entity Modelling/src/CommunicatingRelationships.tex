...
In this case the relationship $r$ on the model can be annotated with the text `intra-department'
in place of `inter-department'. 
...
\end{tabular}
\end{erboxedFigure}
You have to look carefully to see the difference but the model re-annotated looks like this.
\begin{equation}
\label{shlaerLang-DeptStudentProfessor..collapsedReferential..diagram}
\scalebox{0.9}{\input{\handCraftedImagesFolder/shlaerLang-DeptStudentProfessor..collapsedReferential..diagram.tex}}
\end{equation}
What makes all the difference is the annotation which reads \textit{intra-depertment}
...
the relationship $r$ between student and professor is intra-departmental
...
are that only in the context of a department may a name unambigouly identify a particular professor.
...
\begin{worktt}
\subsubsection {A questionably interesting point}
\commentary{Afterthought?}
\commentary{What this section is about is that there are simple facts which are not instances of binary relationships. }
...
\end{equation}
is tautologous and a tautology doesn't tell us  anything.
...
\end{equation}
This prescription is particularly important if student Brown does not have an advisor because the existence of Brown cannot be deduced from her relationship with her advisor. This is perhaps an afterthought. I started by saying that facts to an entity modeller are instances of binary relationships. Here is a fact that isn't of that form and it can't be put into that form except by artificially asserting
\begin{equation}
\label{MathsBrownIsOfTheirSelf}
\mbox{\rdot{Maths} student \rdash{Brown} is of a self with themselves.}
\end{equation}

Basically binary self identity collapses to unary existence. 
\end{worktt}
