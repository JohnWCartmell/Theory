...
\mynote Explain what jointly injective means for a set of functions.
Explain that this is equivalent to the paired function being injective.
Explain that an injective gunction into a cartesian product is the same as a binary relation.
Therefore pairs of barred functional relationships correspond to binary relations.
therefore triples of narred functional relationships coirrespond to ternary relationships.
In the general case if an entity type has exactky $n$ outgoing barred relationships then
we can equate this to an $n-$ary relation. These are what Chen refered to a relationship sets in 1976 and then later in 1983 as relationship types.
...
\raisebox{-2cm}{\input{\ImagesFolder/shlaerMellorDeptStudentProfessor0.tex}}
\end{equation}

\commentary{Correct above diagram. Move left and also majors in $\leftarrow$ majoring in}
