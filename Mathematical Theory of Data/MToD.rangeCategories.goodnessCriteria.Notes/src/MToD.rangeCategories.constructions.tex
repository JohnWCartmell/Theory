...
and $a$ is an object of \catcw then the functor $Hom(a,-)$ is faithful.
\end{lemma}
\begin{proof} \commentary{CHECK THIS PROOF}
Let $j_1,j2: x \morph y$ in \catcw and suppose that
\begin{equation}
\label{faithassumption}
HomP(a,j_1)=HomP(a,j2).
\end{equation} 
We need show that $j_1=j_2$. First we show that 
\begin{equation}
\label{faithstepone}
...
\begin{align*}
j1 &= HomP(a,j_1)(\bar{j_1}) &&\mbox{by definition of $HomP$, } \\
   &= HomP(a,j_2)(\bar{j_1}) &&\mbox{by assumption (\ref{faithassumption}), } \\
   &= \bar{j_1} \circ j_2    
...
\commentary{True of any functor that preserves the support operation??.}
\begin{newtt}
...
\begin{lemma}
\llabel{HomOfEffbargbar}
\fgsourcediag
\begin{align*}
[L_c(f)] &= [L_a(Hom(a,f)(\bar{f}\circ\bar{g}))] \\
[L_c(g)] &= [L_a(Hom(a,g)(\bar{f}\circ\bar{g}))]
\end{align*} \commentary{We use this later on we need to prove it.}
\end{lemma}
\begin{proof}
\end{proof}
