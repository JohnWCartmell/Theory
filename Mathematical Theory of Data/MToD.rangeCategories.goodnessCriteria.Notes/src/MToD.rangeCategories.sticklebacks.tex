...
\paragraph*{key observation xxxx}
\begin{lemma}
\llabel{xxxxx}
...
\begin{enumerate}[(i)]
\item
\item if $F::\tuple{j}$ is defined for some morphism $j$ then
$\tighten{F \cnc \rst{j}}= \tighten{\tighten{F} \cnc \rst{j}}$.
\end{enumerate}
\end{lemma}
\begin{proof}
~
...
Then by yyyyy, $H \leq \tighten{F} \circ \rst{j}$. But then because $H$ is tight then $H \leq \tighten{\tighten{F} \circ \rst{j}}$. 
\end{enumerate}
\end{proof}
\paragraph*{alpha}
\begin{lemma}
...
for some morphism $j$, 
then the final element of $\tighten{F \cnc \tuple{g}}$ 
is $f_n \circ g$. 
\end{enumerate}
\end{lemma}
\begin{proof}
