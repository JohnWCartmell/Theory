...
\newcommand{\tighteningLeft}{\Tighten{ F \cnc \rstdot{\tighten{G}}}}
\newcommand {\tighteningRight}{\Tighten{ \rngdot{\tighten{F}} \cnc G}}
\newcommand {\theTightening}{\tighteningLeft \cnc \tighteningRight}
\begin{lemma}\commentary{\workt{Need} rstdots, rngdots and tuples. \workt{Yuk.}}
...
$Z_1$ and $Z_2$ such that the concatenation $Z_1 \cnc Z_2$ is defined then $Z_1 \cnc Z_2$ is tight iff $\rng{Z_1} = \rst{Z_2}$. It suffices, therefore to show that 
\[
\rng{\tighteningLeft} = \rst{\tighteningRight}
\] 
This is the case because by lemma \lref{beta} both left and right hand side are equal to 
$\rng{\tighten{F}} \circ \rst{\tighten{G}}$.
...
\rng{\rst{h} \circ f_n} \leq \rngdot{\tighten{F}}
\end{equation}
...
(by (\ref{rstOfhL}) and since $\rst{h_n} = \rng{h'_{n-1}}$
 as $H$ is tight)
then we get
\begin{equation}
...
This is supports the left hand side of our target ineequality, for the right hand side we
put(\ref{inequalityOneRight}) 
and (\ref{inequalityThreeLeft}) together to get
\[
\tuple{\rng{\rst{h} \circ f_n}} \cnc \zigzagRightFactorTailTupleLedIn{h}{n}{m}{g_1} 
