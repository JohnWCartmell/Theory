...
and  
\[
h_R = \rng{\rst{h_n} \circ f_n} \circ g_1 \circ \rng{h_n}.
\]
...
which is to say
\begin{equation}
\label{inequalityFourLeftRewritten}
...
\zigzagTupleReplaceOut{h}{n}
        {h_L}
              \leq F \cnc \rstdot{\tighten{G}}
\end{equation}
and since the left hand side is tight
\begin{worktt}
(since 
  $\rst{{f_n \circ \tuple{\rst{ g_1 \circ \rng{h_n}}}}}
                   = \rst{h_n} = \rng{h'_{n-1}}$
)
\end{worktt}
then we get
...
\end{equation}
and similarly using \workt{(\textit{such a thing})} (\ref{inequalityOneRight}) 
and (\ref{inequalityThreeLeft}) we get
\begin{equation}
...
Putting (\ref{inequalityFiveLeft}) 
and (\ref{inequalityFiveRight}) together
we can  simplify as  $h_L \circ h_R = h$ and we get 
\[
H \leq \theTightening,
\]
...
For above proof see small whiteboard.
\begin{oldtt}
