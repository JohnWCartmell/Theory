...
We will show that every zigzag $F$ has a tightening and w2e will then write $T(F)$ for the tightening of $F$..
...
\ncarrrecursive{y}{y}{80}{170}{10}\alabel{\widehat{j}}
\end{arrows} 
\] 
in \catcw then if $\rst{z} \leq \rst{i}$,
$z \leq f$ and $\widehat{z} \leq \widehat{j}$ then
$z \leq \rst{i} \circ f \circ \widehat{j}$.
\end{observation}

That every zigzag $F$ has a tightening will follow by induction from the following lemma
\begin{lemma}
\llabel{leftToRightTighteningSubLemma}
Every zigzag has a tightening. 
\end{lemma}
\begin{proof}
Prove by induction on the length of the zigzag.
If the zigzag is of length $1$ then it has no zigs and no zags so is already tight.
...
Now suppose  $F=\zigzagTuple{f}{n}$ is a zigzag in \catcw.
By the inductive hypothesis there is a tightening
 $\zigzagLeadingTuple{t}{n}$ of the tuple 
\begin{equation}
...
{\tuple{t_1,t'_1,...t'_{n-2},t_{n-1},\; 
    \rst{f_n} \circ f'_{n-1} \circ \widehat{t_{n-1}},\; 
    \rst{f'_{n-1} \circ \widehat{t_{n-1}}} \circ f_n
    }
}
We show that this zigzag
\begin{equation}
\label{requiredTightening}\requiredTightening
\end{equation}
is the tightening of $F$. We have to show first that this is tight. For this we need the following idenities:
\begin{align}
\label{tightnessIdentityOne}
\reallywidehat{t_{n-1}} &= \reallywidehat{\rst{f_n} \circ f'_{n-1} \circ \widehat{t_{n-1}}}  \\[0.1cm]
\label{tightnessIdentityTwo}
\rst{\rst{f_n} \circ f'_{n-1} \circ \widehat{t_{n-1}}}&=\rst{\rst{f'_{n-1} \circ \widehat{t_{n-1}}} \circ f_n}
\end{align}

(\ref{tightnessIdentityOne}) is the case  
because its rhs simplifies to $\reallywidehat{\rst{f_n} \circ f'_{n-1}} \circ \widehat{t_{n-1}}$
and,
since
$
...
of (\ref{inductionPrecursor}), we have
\begin{align*}
\widehat{t_{n-1}} &\leq  \reallywidehat{
       f_{n-1} \circ 
       \reallywidehat{
                \rst{f_n} \circ f'_{n-1}
                     }
                                      }\\
         &= \reallywidehat{f_{n-1}}
          \circ 
       \reallywidehat{
                \rst{f_n} \circ f'_{n-1}
                     }\\[0.1cm]  
    &\leq  \reallywidehat{\rst{f_n} \circ f'_{n-1}}. 
\end{align*}

(\ref{tightnessIdentityTwo}) holds because both sides of the identity simplify, using RR.3, and R.2, to $\rst{f'_{n-1} \circ \widehat{t_{n-1}}} \circ \rst{f_n}$.

Having shown that (\ref{requiredTightening}) is tight,  
we can  show that it is the tightening of $F$ by showing that it dominates any tight zigzag $Z$ such that
$Z \leq F$ 
...
Now, this holds for the initial segments upto and including
the inequality $z'_{n-2} \leq t'_{n-2}$ just because $\zigzagLeadingTuple{t}{n}$
 is to be the tightening of
 $\tuple{\tightlead}$. 
...
\begin{align*}
z_{n-1} &= z_{n-1} \circ \reallywidehat{\rst{z_n} \circ z'_{n-1}} && \mbox{trivially, since $Z$ is tight}\\
&\leq  f_{n-1} \circ 
       \reallywidehat{
                \rst{f_n} \circ f'_{n-1}
...
For the final two components of the target inequality (\ref{targetInequality}) we need to show
 \begin{equation}
 \label{inequalityTwo}
 z'_{n-1} \leq \rst{f_n} \circ f'_{n-1} \circ \widehat{t_{n-1}}
 \end{equation}
 and
  \begin{equation}
 \label{inequalityThree}
 z_n \leq \rst{f'_{n-1} \circ \widehat{t_{n-1}}} \circ f_n
 \end{equation}

The first holds because
\begin{align*}
z'_{n-1} &= \rst{z'_{n-1}} \circ z'_{n-1}  \circ \widehat{z'_{n-1}} 
                && \mbox{trivially, by R.1 and RR.2,}\\
         &= \rst{z_n} \circ z'_{n-1}  \circ \widehat{z_{n-1}} 
         &&\mbox{because $Z$ is tight,}\\
         &\leq \rst{f_n} \circ f'_{n-1}  \circ \widehat{t_{n-1}}
         &&\mbox{because $Z \leq F$ and (\ref{inequalityOne}).}
\end{align*}
...
\end{lemma}
\begin{proof}
\begin{worktt}This is trivial now. IF it still has reason to exist then make into an observation.
\end{worktt}
...
If $F=\zigzagTuple{f}{n}$ is a zigzag of length $n \ge 2$ then
\[
T(F)=T(\tuple{f_1,f'_1,...f_{n-1},\rst{f_n} \circ f'_{n-1},f_n})
\]
\end{lemma}
\begin{proof}
Let $F'=\tuple{f_1,f'_1,...f_{n-1},\rst{f_n} \circ f'_{n-1},f_n}$
...
\end{newtt}
\begin{aside}
An adjunction to this would be:
\[
T(F::\tuple{g}) \leq T(F) :: \tuple{g}
\]
...
This is a direct consequence of
 lemma \lref{leftToRightTighteningSubLemma}. Make it a corollary, possibly with parts (i) and (ii).
\end{proof}

\begin{worktt}
 I should factor out  a midpoint lemma here from
the later tighteningCommutesWithComposition Lemma  so that whenever 
...
To establish (a), since $T(T(F) :: T(G))$ is tight,
by corollary \lref{absoluteTighteningCorollary}\commentary{I feel this should be the defintion of tightening}
it suffices to show that 
$T(T(F) :: T(G)) \leq F:: G$. 
and, since $T(T(F) :: T(G)) \leq T(F) :: T(G)$, it therefore suggices to show that
\[
T(F) :: T(G) \leq F:: G
\]
i.e. that 
...
\end{proof}
\begin{aside}
I feel that the definition of tightening should be closer to that there
corollary \lref{absoluteTighteningCorollary}. Then that a tightening always exists should be proved by induction.
\end{aside} 
...
\[
\rst{f_0} \leq \rst{f'_0}
\]
i.e. 
\begin{equation}
\label{leftLowInequality}
\rst{f_0} \circ \rst{f'_0} = \rst{f_0}
\end{equation}
and
\begin{equation}
...
T(\tuple{f_0,f'_0,f_1})=\tuple{f_0,
                                 \rst{f_0} \circ f'_0,
                                 f_1 \circ \widehat{\rst{f_0} \circ f'_0}
                                }
\]
\end{lemma}
\begin{proof}
...
\begin{align*}
t(\tuple{f_0,f'_0,f_1}) 
                &= \tuple{f_0 \circ \rst{f'_0}, 
                          \rst{f_0} \circ f'_0 \circ \widehat{f_1},
                           f_1 \circ \widehat{f'_0}
                          }     
                    && \mbox{by following the definition of $t$,}    \\
                &= \tuple{f_0, \rst{f_0} \circ f'_0,f_1} 
...
t^2(\tuple{f_0,f'_0,f_1})
           &= \tuple{
                    \rst{\rst{f_0} \circ f'_0} \circ f_0,
                    \rst{f_0} \circ \rst{f_0} \circ f'_0 \circ \reallywidehat{f_1},
                    \reallywidehat{\rst{f_0} \circ f'_0} \circ  f_1 
                    }
           &&  \mbox{by applying the definition of $t$ to $\tuple{f_0, \rst{f_0} \circ f'_0,f_1}$,} \\
           &= \tuple{f_0, 
                     \rst{f_0} \circ f'_0,
                     \reallywidehat{\rst{f_0} \circ f'_0} \circ  f_1 
                     } 
...
\end{align*}
Finally, the zig of $\tuple{f_0, \rst{f_0} \circ f'_0,f_1 \circ \widehat{\rst{f_0} \circ f'_0}}$
is tight because we have that  (\ref{leftLowInequality})
and  the zag is tight because
\[ 
  \reallywidehat{\rst{f_0} \circ f'_0} = \reallywidehat{f_1 \circ \widehat{\rst{f_0} \circ f'_0}}
\]
because handwritten sheet A
\begin{align*} 
\reallywidehat{\rst{f_0} \circ f'_0} 
                &=  \widehat{f'_0} \circ \reallywidehat{\rst{f_0} \circ f'_0} 
                &&\mbox{lemma  \ref{fglemma} (ii)}\\
                &= \widehat{f_1} \circ \reallywidehat{\rst{f_0} \circ f'_0}
                &&\mbox{by (\ref{leftLowEquality})} \\ 
                &= \reallywidehat{f_1 \circ \widehat{\rst{f_0} \circ f'_0}}
...
T(\tuple{f_0 \circ g,f'_0,f_1})=\tuple{f_0 \circ g,
                                 \rst{f_0 \circ g} \circ f'_0,
                                 f_1 \circ \widehat{\rst{f_0 \circ g} \circ f'_0}
                                }.
\]
\end{corollary}
\begin{proof}
The lemma can be applied to  $\tuple{f_0 \circ g,f'_0,f_1}$ because of the tightness of  $\tuple{f_0,f'_0,f1}$ and
because $\rst{f_0 \circ g} \leq \rst{f_0} = \rst{f'_0}$.
\end{proof}
\end{oldtt}
...
If $F=\zigzagTuple{f}{n}$ is a morphism in $Z_C$ then define $\rst{F}$ to be $\tuple{\widehat{f_0}}$ the  zigzag of length $1$ determined by the morphism $\widehat{f_0}$ of \catcw.
...
\[
\rst{F \circ G} = \rst{F} \mbox{ implies  } F \circ \rst{G} = F.
\]
...
\begin{align*}
\rst{\rst{F} \circ G} 
                   &= \rst{T(\tuple{g_0 \circ \widehat{f_0},g'_0,...g_m})}\\
                   &= \rst{\tuple{g_0 \circ \widehat{f_0},s'_0,...s_m}}
...
\begin{align*}
\rst{F \circ G} &= \rst{F \circ \bar{\bar{G}}} \\
                     &= \rst{F \circ \bar{G}} && by ??
\end{align*}



\begin{lemma} \commentary{is this used?}
if f composes with g in range category \catcw then
\[
...
since $ff'_{i_s} = \rst{f_{i_s+1} \circ \reallywidehat{ff'_{i_s+1}   }} \circ f_{i_s}$ and
...
\[
\reallywidehat{\rst{gg'_{j_s-1}} \circ g_{j_s} \circ s} 
= \reallywidehat{\rst{g'_{j_s} \circ\reallywidehat{\rst{gg'_{j_s-1}} \circ g_{j_s}}} \circ  g_{j_s + 1} } 
\]
This follows from lemma \lref{tighteningashorteninglemma}
...
$g'_{j_s}$ for $g$,
$g_{j_s+1}$ for $h$,
and is applicable because $\widehat{g_{j_s}} = \widehat{g'_{j_s}}$
because $G$ is assumed to be tight.

If $j_s=1$ we need show that
\[
\reallywidehat{ f_n \circ g_1 \circ s} 
= \reallywidehat{\rst{g'_1 \circ\reallywidehat{ f_n \circ g_1}} \circ  g_{2} } 
\]
This follows from lemma \lref{tighteningashorteninglemma}
 (\lref{tighteningshorteningLeftToRight})
by removing the $\bar{i}$ term (i.e. by substituting the appropriate identity morphism, $id_{w_n}$)
and by
substituting
$f_n \circ g_1$ for $f$,
$g'_1$ for $g$ and
$g_2$ for $h$
and is applicable because $\widehat{f_n \circ g_1} \leq
\hat{g_1} = \hat{g'_1}$.
...
\end{lemma}
\begin{proof}
For any series of shortenings of $F$ to a single morphism one of this set must shorten the triange the final zag in F i.e. 
...
at morphisms $s_1,...s_{n-1}$ which shorten $F$ to a singleton (and therefore short) zigzag $\tuple{f_1 \circ s_1}$. But since the shortening of a tight zigzag is unqiue 
