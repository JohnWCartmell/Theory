...
position $i_s$ where
$i_s \leq  n-2$ and if $g$ is a morphism such that 
\[
T(shorten(F\cnc\tuple{g},i_s,s))=shorten(T(F\cnc\tuple{g}),i_s,s))
\]
\end{lemma}
\begin{proof}
...
LHS: First 
\[
shorten(F\cnc\tuple{g},i_s,s) = shorten(F,i_s,s) \cnc \tuple{g}
\]
\\
RHS:
\\ 
By lemma \lref{leftToRightTighteningObservation} there is a tuple
$(F\cnc\tuple{g})^L$ such that \highlight{YUK}
\[
T(F\cnc\tuple{g}) = T((F\cnc\tuple{g})^L) \cnc T( \tuple{\rng{T((F\cnc\tuple{g})^L)},f'_{n-1},f_n \circ g} )  
\]



and by induction we have (a)
\[
shorten(T((F\cnc\tuple{g})^L),s,i_s) = T(shorten((F\cnc\tuple{g})^L),s,i_s))
\]

and also we have (b)
\begin{align*}
T(shorten((F\cnc\tuple{g})^L)&,s,i_s)) 
           \cnc T( \tuple{\rng{F\cnc\tuple{g}},f'_{n-1},f_n \circ g} ) \\  
  &= T(shorten((F\cnc\tuple{g})^L),s,i_s) \cnc \tuple{\rng{F\cnc\tuple{g}},f'_{n-1},f_n \circ g})
     && \mbox{\parbox[t]{4cm}{because tightening commutes with composition albeit $::$}}\\
  &= T(shorten((F\cnc\tuple{g})
     && \mbox{\parbox[t]{4cm}{because for any $Z$ of length $>1$, $Z^L :: Z^{L_r} = Z$ }}
\end{align*}

providing we have 
\[
shorten((F\cnc\tuple{g})^L),s,i_s) \cnc \tuple{\rng{F\cnc\tuple{g}},f'_{n-1},f_n \circ g}
= shorten((F\cnc\tuple{g})
\]
which maybe we do if we have
\[
\rng{shorten(Z)} = \rng{Z}
\]

Therefore from (a) and (b) we have (c) that
\[
shorten(T((F\cnc\tuple{g})^L),s,i_s) \cnc T( \tuple{\rng{F\cnc\tuple{g}},f'_{n-1},f_n \circ g} ) 
         = T(shorten((F\cnc\tuple{g}) \mbox{which is LHS of target}
\]
and looking at the lhs of (c)  we have is we look carefully we have
\begin {align*}
shorten(T((F\cnc\tuple{g})^L),s,i_s) \cnc T( \tuple{\rng{F\cnc\tuple{g}},f'_{n-1},f_n \circ g} ) 
&= shorten(  T((F\cnc\tuple{g})^L :: T( \tuple{\rng{F\cnc\tuple{g}},f'_{n-1},f_n \circ g} )   ,s,i_s)\\ 
&= shorten(T(F\cnc\tuple{g})^L \cnc  \tuple{\rng{F\cnc\tuple{g}},f'_{n-1},f_n \circ g} ),s,i_s)\\
&= shorten(T(F \circ g), s , i_s) \\
&= \mbox{RHS of target}
\end{align*}

\begin{aside}
Should I write  $T((F \cnc \tuple{g})^L)$ instead of $T((F \cnc \tuple{g})^L)$.

and then write  $F= F^{L} \cnc F^{L_r}$

and thus $F\cnc\tuple{g} = (F\cnc\tuple{g})^{L} \cnc (F\cnc\tuple{g})^{L_r}$??

\workt{Yes}
Think also curly T for tighten and curly $S_{i}$ for shortening at $i$.

for example $\mathcal{T}(\mathcal{S}_{i_s}(F\cnc\tuple{g},s))$

Use macros so that we can change later.
\verb!\tighten{F}!
\verb!\shorten{F}{s}{i_s}!
\end{aside}
...
\begin{align*}
LHS &= \tighten{\shorten{F\cnc\tuple{g}}{s}{i_s}} \\
    &= shorten(T((F\cnc\tuple{g})^L),s,i_s) \cnc T( \tuple{\rng{F\cnc\tuple{g}},f'_{n-1},f_n \circ g} ) \\
    &= shorten(  T((F\cnc\tuple{g})^L :: T( \tuple{\rng{F\cnc\tuple{g}},f'_{n-1},f_n \circ g} )   ,s,i_s)\\
    &= \shorten{\tighten{F\cnc\tuple{g}}^L \cnc  \tuple{\rng{F\cnc\tuple{g}},f'_{n-1},f_n \circ g} }
    {s}{i_s} \\
    &= \shorten{\tighten{F\cnc\tuple{g}}}{s}{i_s} \\
    &= RHS
\end{align*}
