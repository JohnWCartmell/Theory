...
The conacatenation of zigzags in the above lemma has a significant feature

\begin{aside}
We use the symbol $\cnc$ to denote concatenation of zigzags. The three dots are arranged along the same rising diagonal as the morphisms in our diagrams. When concatenation is followed by shortening at one end, we use $\cnctail$ or $\headcnc$ to indicate that the corresponding end element has disappeared. Thus, if $G$ begins with the restriction idempotent $\rst{G}$ and
\[
\rst{G}=\rng{F},
\]
then the concatenation may be written
\[
F\cnc G = F\cnctail G.
\]
\end{aside}
...
\shorten{\tighten{F}}{s}{i}
   ...
   &= \mathrlap{
       ...
       &&& \mbox{\parbox{6cm}
                {
                  by lemma 
                  \lref{howToTightenAThreefoldConcatenationTwo},
                  where $L= \rightPartOneOfThree$\\
                  $C=\ZZ$\\
                  and $R=\rightPartThreeOfThree$,
                }
                } \\
    &= \mathrlap{
               \rightPartOneOfThree
