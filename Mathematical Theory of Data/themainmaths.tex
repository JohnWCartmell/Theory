...
Note that $\pk \subseteq \rkp$ in spirit but not in fact?. \commentary{Check this in regard to identifying attributes.}
\begin{definition}
...
\highlight{$\firstcutmodel$}. Generally this first cut relational model will not be alias-free and so it can be improved upon by trimming aliases. The trimmed model we shall denote by 
...
\end{equation*}
We define the model $\chigmodel$  \index[definitions]{$\chi$}  be a further improvement to the first-cut transformation obtained by excluding  from  the transformed model $\firstcutimproved(\gmodel)$ those attributes $\qq{r \circ q}$ for which either there exists a \sout{unique}  path $s$ such that  $r \circ q < s$ or for which there exists a path $s$ which is not a relationship key path and 
