...
\end{proof}
\subsection{\highlight{Relationship-like} Paths}
\begin{definition}
...
\commentary{defined 11th December 2019}
either  $t$ is the empty path (so that the path $p$ is exactly a relationship $R$) or $t$
is identifying-like.\commentary{A \highlight{relationship-like} path, as a relationship i.e. a path of length 1,  can be navigated in SQL without use of intermediate tables (only true in a first-cut model!!) . Paths which have length greater than one and are not \highlight{relationship-like} need be navigated in SQL using intermediate tables.}
\end{definition}

\begin{lemma}
\llabel{relationshippreservingmappingspreserverelationshiplike}\commentary{not sure that this is required}
If $\gmodel$ and $\gmodel'$ are ER models and  if  $\varphi$ is ER model mapping, $\varphi: \gmodel \morph \gmodel'$  which preserves relationships  i.e. if relationships of $\gmodel$ are mapped to relationships of $\gmodel'$ by  $\varphi$, then \highlight{relationship-like} paths are preserved by $\varphi$.
\end{lemma}
\begin{proof}
Assume such a mapping $\varphi$, and assume that a path $p$ of $\gmodel$ is \highlight{relationship-like}.
From the definition of \highlight{relationship-like}, either $p$ is exactly a relationship in which case it is preserved by $\varphi$ by initial assumption or $p$ consists of path $R \circ t$ where $t$ is an identifying-like path.
In the latter case, from the definition of mapping, $\varphi(p)$ consists of $\varphi(R)$, which is a relationship in $\gmodel'$, from the initial assumption, followed by a path $\varphi(t)$ which is identifying-like since $t$ is, by lemma \lref{mappingspreserveidentifyinglike}. Therefore $\varphi(p)$ is \highlight{relationship-like} in $\gmodel'$.
\end{proof}
...
then $\varphi$ is surjective on \highlight{relationship-like} paths of $\gmodel'$. 
\end{lemma}
\begin{proof}
If $r':\varphi(a) \morph \varphi(b)$ in $\gmodel'$ is \highlight{relationship-like} then it consists of a relationship $R': \varphi(a) \morph \varphi(a')$, for some $a'$,
...
The path $r=R \circ t$ is a \highlight{relationship-like} path such that $\varphi(r)=r'$ as required.
\end{proof}
...
each leaf primary referential inclusion dependency of $\gmodel$ is represented in $\gmodel$ by some \highlight{relationship-like} path $p$, 
}
\end{reinstatett}
\item{
...
\begin{reinstatett}
\item{ 
That each leaf primary referential inclusion dependency of $\gmodel'$ is represented in $\gmodel'$ by some \highlight{relationship-like} path $p$
...
(lemma \lref{isospreserveandreflectreferentialincds})
 and because  representation by a path is preserved (lemma \lref{isospreserveandreflectrepresentationsofincds}) and \highlight{relationship-like} paths are preserved
...
\begin{definition}
\noindent An ER model $\gmodel$ has the \term{\fdfactoring} property iff to every functional dependency $\set{x_1,...x_n} \morph z$ where $x1, x_2, ... x_n$ \oldt{and $z$} are \reinstatet{\attributelike} \commentary{\reinstatet{reinstated}}paths either $\set{x_1,...x_n}$ is
...
See the same example for a model in which the fd factoring property holds but there is an intransitive functional dependency which cannot be factored intransitively on the right and in which the lhs is not a mono-source but which is not attribute-like.\commentary{Double check this}
\end{examples}
...
in $\gmodel$. \commentary{\newt{need attribute-like reflected?}}Since $\gmodel$ is assumed to have the fd factoring property either $\set{\varphi^{-1}(x'_1),...\varphi^{-1}(x'_n)}$ is a mono-source in $\gmodel$ in which case $\set{\xnprime}$ is a mono-source in 
...
$sfd{\xn}{z}$ is a non-trivial (\reinstatet{leaf}) functional dependency sourced
at some entity type $a$ then either $\set{\xn}$ is a mono-source in $\gmodel$
or else there is an entity type $d$ and there is a \reinstatet{relationship-like} path $p:a \rightarrow d$ such that $\{x_1,...x_n\} \morph p$
...
\begin{definition}
An ER model $\gmodel$ is said to be \term{fixed property factored} iff whenever $p: a \morph a$ is a non-empty path in $\gmodel$ 
...
\commentary{ This definition coming about remember because we are trying to prove lemma 
...
\item the relationship $r_1$ is a mono-source and the relationship $r_2$ is not identifying-like, or
\item the attribute $y$ is identifying.
\end{enumerate}
\end{newtt}
\end{definition}

In a relational ER model $\gmodel$ if there are no identifying attributes $x$ such that $x \simeq r \circ p$ where $r$ is \highlight{relationship-like}, $r:a \morph a'$, and $p$ is a not a primary key path, $p:a' \morph \veee$ then the model is said to be \term{without caveats}.

Thus we get:

\begin{definition} 
An ER model $\gmodel$ is \term{reduced without caveat} iff for all attributes $x:a \morph \veee$
such that there is a non-empty path $r:a \morph b$ and a \newt{non-empty} path $p:b \morph \veee$ such that $x \simeq r \circ p$ it follows that 
either \commentary{\newt{Rewrite this in line with above}}
\begin{enumerate} [(i)]
\item \newt{the path $r$ is relationship-like} and there exists a primary key path \commentary{an identifying attribute} $q:b \morph \veee$ such that $p \simeq q$, or
...
where $x_1,...x_n$, $y$ are \highlight{attributes} 
...
or else there is a non-empty \oldt{relationship-like} path $r_1 \circ r_2 ... \circ r_n$ 
of length $n$ in $\gmodel$, for some $n \geq 1$  where each $r_i$ is a relationship,
and where $r_2 \circ ... r_n$ is identifying-like, \commentary {need $r_2$ to be identifying-like}
and a path $w$ of $\gmodel$ 
such that the path $r_2 \circ ... \circ r_n \circ w$
 is not  equivalent to a primary key path and such that 
\begin{equation}
\label{simpleRepresentation}
 p \circ w \simeq y
\end{equation}
and such that $\msfd{x_1,...x_n}{r_1 \circ r_2 ... \circ r_n}$. 

If the model $\gmodel$ is assumed to be reduced without caveat then from (\ref{simpleRepresentation})
and since $r_2 \circ ... \circ r_n \circ w$
 is not  equivalent to a primary key path then it follows that he relationship $r_1$ must be a mono-source
and the relationship $r_2$ must be non-identifying which is a contradiction so we must assume that $n$ 
equals $1$.

Now it follows that since $r_1$ is a mono-source then $x_1,...x_n$ is a mono source, 
since $\msfd{x_1,...x_n}{r_1}$ \newt{by lemma \ref{fdrightmonosourceimpliesfdleftmonosource}}, as required and we have established BCNF.
...
such that there is a \oldt{relationship-like} newt{non-empty} path $r:a \morph b$ and a \newt{non-empty} path $p:b \morph \veee$ such that$x \simeq r \circ p$ it follows that 
either 
\begin{enumerate} [(i)]
\item \newt{the path $r$ is relationship-like} and there exists a primary key path  $q:b \morph \veee$ such that $p \simeq q$, or
...
\end{lemma}
\begin{proof}
Suppose that in the model $\chigmodel$ there is an attribute $x:a \morph \veee$, a \highlight{relationship-like} path $r:a \morph b$ and a path $p:b \morph \veee$ such that$x \simeq r \circ p$ we need show that either 
\begin{enumerate} [(i)]
...
In the first case it follows that there is a \highlight{relationship-like} path $r$ in $\gmodel$ 
...
\newt{is leaf restriction free and is reduced} 
\oldt{and is free of non-identifying referential attributes}
then the transformed model $\chigmodel$ is a  relational ER model that is well-formulated, \oldt{simple}\newt{reduced}, leaf-restriction free
and has the fd factoring property.   
Further if $\gmodel$ is \oldt{free of any referential attributes at all} \newt{reduced without caveats} then  $\chigmodel$ is simple without caveats.
\end{lemma}
\begin{proof}
...
$\chigmodel$ is leaf-restriction free by \newt{by lemma \lref{restrictioncarrythroughlemma}.}
\begin{oldtt}
lemma \lref{wellformulatedandfreeofdesignredundancyimpliesleafrestrictionfree}  since $\gmodel$ is assumed
to be  well-formulated, to have the fd factpring property and to be free of design redundancy.
\end{oldtt}

$\chigmodel$ is \newt{reduced bu=y lemma??? }\oldt{simple by lemma \lref{simplecarrythroughlemma}}  because $\gmodel$ is \newt{reduced}\oldt{because this follows directly from the definitions because, it is assumed, it is free of non-identifying referential attributes}.
...
\begin{enumerate}[(i)]
\item  if $\gmodel$ is \newt{reduced} \oldt{free of non-identifying referential attributes} then  $\chigmodel$ is in third normal form, 
\item 
if $\gmodel$ is \newt{reduced without caveats}\oldt{free of any referential attributes at all} then  $\chigmodel$ is in BCNF.
\end{enumerate}
\end{theorem}
\begin{proof}
