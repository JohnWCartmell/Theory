...
If $\gmodel$and $\gmodel'$ are ER models and if $\varphi$ is an isomorphism between $\gmodel$ and $\gmodel'$ then $t: a  \morph  b$ is an identifying-like path in $\gmodel$ iff $\varphi(t)$ is an identifying-like in $\gmodel'$.
\end{lemma}
\begin{proof}
...
$\varphi(t)=t'$ and this $t$ is identifying by lemma \ref{isospreserveandreflectidentifyinglike}.  
\end{proof}
...
If $\gmodel$ and $\gmodel'$ are ER models and  if  $\varphi$ is an isomormorphism, $\varphi: \gmodel \morph \gmodel'$, that is surjective on relationships   which preserves relationships  i.e. every relationship of $\gmodel'$ is the mapping of a relationship of $\gmodel'$ by  $\varphi$, 
then $\varphi$ is surjective on relationship-like paths of $\gmodel'$. 
\end{lemma}
\begin{proof}
Use lemma \ref{isossurjectiveonrelationshipsaresurjectiveonidentifyinglike}.
\end{proof}


Finally note that \colorbox{yellow}{key-realising} isomorphisms\commentary {what is this all about?} reflect primary mono-sources as described by this
lemma.

keyrealisingisosreflectprimarymonosources
\begin{lemma}
\label{keyrealisingisosreflectprimarymonosources}
If $\varphi: \gmodel \morph \gmodel'$ is a \colorbox{yellow}{key-realising} isomorphism of ER models then
...
If $\gmodel$ and $\gmodel'$ are ER models and  if  $\varphi$ is a \colorbox{yellow}{key-realising} isomorphism between $\gmodel$  and $\gmodel'$ that preserves relationships\footnote{meaning that it maps relationships to relationships} and reflects primary mono-sources then if $\gmodel$ is well-formulated then $\gmodel'$ is well-formulated.
\end{lemma}
\begin{proof}
...
\begin{definition}
In a relational ER model $\gmodel$ an attribute $x: a \morph \veee$ is said to be \term{redundant} if
...
If $\gmodel$ is $\chit$-suitable  then $\chigmodel$ there is a \colorbox{yellow}{key-realising} and
...
The converse has NOT BEEN ESTABLISHED.\commentary{PROBLEM ---- NOT PROVEN}
\end{proof}
