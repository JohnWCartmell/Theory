...
\item
If $a$ is a node of a directed graph $G$ then by a \term{source} at $a$ in $G$ we mean a non-empty \newt{multiset} of paths in $G$ each with domain $a$.
\end{itemize}
\begin{figure}
...
\subsection{Equivalence of \newt{sources} \oldt{sets of paths}}
\begin{definition}
If $\gmodel$ is a model and if $P$ and $P'$ are \newt{sources} in $\gmodel$ then say that
...
If $P$ is a \newt{source} of an ER model $\gmodel$ and $\varphi : \gmodel \morph \gmodel'$ is an ER model mapping then we denote by $\varphi(P)$ the \newt{source} that is the multiset
...
\begin{lemma}
\llabel{mappingspreserveequivalence}
If $\varphi: \gmodel \morph \gmodel'$ is a mapping of ER models and
if $p$ and $q$ are paths of $\gmodel$ such that $p \simeq q$ in $\gmodel$ then $\varphi(p) \simeq \varphi(q)$ in $\gmodel'$. If $P$ and $Q$ are \newt{sources} and $P \simeq Q$ then $\varphi(P) \simeq \varphi(Q)$.
\end{lemma}
\begin{proof}
...
It follows on from the definition of equivalence for \newt{sources} \oldt{sets of paths} $P$ and $Q$ that $\varphi(P) \simeq \varphi(Q)$ if $P \simeq Q$.
\end{proof}
...
We shall denote the extended model by $\gmodel + \qq{P}$. \commentary{Maybe not since there may be paths in $\gmodel -P$ 
included in the equivalence class $\qq{P}$.}
...
\begin{definition}
If $P$ and $Q$ are \newt{sources} of model $\gmodel$ then say that the \newt{source} $P$ 
...
a set of paths $P$ to be \term{\newt{skeletal}\oldt{representative}} iff for all $p,p' \in P$,
...
\begin{definition}
If $P$ and $Q$ are \newt{sources} \oldt{sets of paths} of a model $\gmodel$ then 
say that the set of paths $P$ \term{is representative of} $Q$ iff $P \simeq Q$ and $P$ is \newt{skeletal}. 
\end{definition}
...
\item whenever a \newt{source} $P$ is a leaf primary mono-source of $\gmodel$ then no proper subset of $P$ is a mono-source.
\end{enumerate}
\end{definition}
...
\end{lemma}
\begin{proof}
It follows from the definition of \oldt{set} equivalence \newt{of sources} that if $P$ and $Q$ are \newt{sources} in a model $\gmodel$ and if $P \simeq Q$ in $\gmodel$ then in each defining instance $E$ of $\gmodel$, the set of functions $\setsuchthat{E_p}{p \in P}$ is equal to the set of functions $\setsuchthat{E_q}{q \in Q}$ since for
...
follows by lemma \lref{leafprimarymonosourcerepresentativeofpka} because  $\simeq$ is an equivalence relation and therefore $P \simeq \pk{a}$ iff $Q \simeq \pk{a}$ .
\end{proof}
...
it follows that $\varphi^{-1}(\varphi(p)) \simeq \varphi^{-1}(\varphi(p'))$ in $\gmodel$ (lemma \lref{mappingspreserveequivalence}) and therefore $p \simeq p'$ in
...
\begin{lemma}
\llabel{isosreflectskeletalsources}
If $\varphi: \gmodel \morph \gmodel'$ is an \oldt{injective} isomorphism of ER models and
if $P$ is a  \newt{source} in $\gmodel$ then if the \newt{source} $\varphi(P)$ of $\gmodel'$ is \newt{skeletal} 
 then the source $P$ of $\gmodel$ is \newt{skeletal}. 
\end{lemma}
\begin{proof}
Assume such an isomorphism $\varphi$. Suppose such a source $P$ in $\gmodel$ and suppose that $\varphi(P)$ is skeletal in $\gmodel'$.  Suppose that  for some $p,p' \in P$, $p \simeq p'$. Since $\varphi$ is an isomorphism it follows that $\varphi(p) \simeq \varphi(p')$ in $\gmodel'$ (lemma \lref{mappingspreserveequivalence}) and then since $\varphi(P)$ is assumed to be skeletal 
...
\item
\oldt{If $\varphi$ is injective then}
...
\item
follows from lemma \lref{isosreflectskeletalsources}
\end{enumerate}
...
In any ER model $\gmodel$ if $Q$ is a \newt{skeletal source} and if $P \subseteq Q$ and $P \simeq Q$ then $P=Q$.
\end{lemma}
\begin{proof}
...
$\varphi^{-1}(Q') = \varphi^{-1} (Q)$. From this it follows that  $Q' \simeq \varphi(\varphi^{-1}(Q')) = \varphi(\varphi^{-1}(Q)) \simeq Q$ and so we have that both $Q' \subset Q$ and $Q' \simeq Q$ and, since $Q$ is skeletal, this implies, by lemma \lref{representativecontainmentlemma}, that $Q'=Q$ as required. \\
...
\subsubsection{Construction of Primary Keys as Attributes}
...
\commentary{unfortuneate double use of R}
\end{lemma}
\begin{proof}
...
\begin{corollary}
\llabel{inclusiondependencyrestrictioncorollary}
If $\incd{a}{x_1,...x_n}{b}{q_1,...q_n}$ is a referential inclusion dependency in an ER model $\gmodel$ 
...
The further point regarding triviality follows from lemma \lref{mappingspreserveequivalence}.
\end{proof}
...
\begin{newtt}
and $x_i \simeq p \circ q_i$ and if $x_i$ is an attribute and $\gmodel$ is reduced then either $p$ is relationship-like or else $p$ is a mono-source (?) an therefore $\set{\xn}$ is a monos-source?? \commentary{mark up early Aug 2020}
\end{newtt}
.
\end{lemma}
\begin{proof}
...
in $\gmodel$. \commentary{\newt{need relationship key reflected?}}Since $\gmodel$ is assumed to have the fd factoring property either $\set{\varphi^{-1}(x'_1),...\varphi^{-1}(x'_n)}$ is a mono-source in $\gmodel$ in which case $\set{\xnp}$ is a mono-source in 
...
is a functional dependency in $\gmodel'$
and then by lemma \lref{fdequivalence}  that $\mmfd{\xnp}{\varphi(y_1),...\varphi(y_n)}$ in $\gmodel'$
...
$\gmodel$ less the relationships which are not of rank less than or equal to n.\commentary{surely need improve this wording} The definition proceeds by induction as follows:
\begin{definition}
...
\begin{enumerate}[(i)]
\item
\oldt{it is primarily keyed i.e. }all identifying edges are attributes, 
\item \newt{there are no relationships of rank $0$}
\item
all relationships are ranked i.e. there are no relationships of null rank,
\item 
for each \newt{leaf?} primary referential inclusion dependency\footnote{Note that in a relational ER model every primary referential inclusion dependency is simple because each primary key path is an attribute because the only identifying edges are attributes.} there is a relationship\newt{-like path?\footnote{dont want to imply $R \circ I$ needs a relationship alias}} which represents it
...
\item a keyed referential constraint is defined to correspond to\commentary{Not so I think.} every simple primary referential inclusion dependency of $\gmodel$ i.e. to correspond to every relationship of $\gmodel$. 
\end{itemize}
...
Generally\commentary{move down into attribute redundancy}  this first cut relational model will not be alias-free and so it can be improved upon by trimming aliases. The trimmed model we shall denote by 
...
Equivalent attributes are an extreme case of attribute redundancy and this kind of redundancy can be avoided by trimming the model as described in section \lref{trimmingaliases}. That the trimmed first-cut model is a relational model follows from the fact that the first-cut model is relational and the following:
\begin{lemma}
...
\commentary{Refer to lemma \lref{subsetextension}}
