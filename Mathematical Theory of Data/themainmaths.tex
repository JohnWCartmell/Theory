...
\begin{newtt}
\begin{definition}
An ER model $\gmodel$ is \term{restriction free} iff \newt{....}
\end{definition}
\end{newtt}
...
In a well-formulated model $\gmodel$  if $\set{x_1,...x_n} \morph y$  is a non-trivial intransitive \oldt{leaf}
...
\begin{lemma}
\llabel{mainlemma}
In a well-formulated model $\gmodel$ that has the \fdfactoring\ property if $\set{x_1,...x_n} \morph z$ is a non-trivial \oldt{leaf} functional dependency sourced
...
and a  path $w:d \morph c$  such that $y \simeq p \circ w$. If \newt{each $x_i$ has codomain $\veee$ and codomain of $y$,} $c$, is $\veee$ then
...
\end{lemma}
\begin{proof}
By lemma \lref{mainlemma} either $\set{\xn}$ is a mono-source or else there exist paths $p$ and $w$ such that $y=p \circ w$ but this is not so 
since $y$ is an edge and $\gmodel$ is assumed to be free of design redundancy. \commentary{But what if $p$ or $w$ includes $y$?}.
\end{proof}
\end{newtt}
