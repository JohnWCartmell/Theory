...
if $P$ is a  \newt{source} in $\gmodel$ then if the \newt{source} $\varphi(P)$ of $\gmodel'$ is \newt{skeletal} 
 then the set of paths $P$ of $\gmodel$ is \newt{skeletal}. 
\end{lemma}
\begin{proof}
Assume such an isomorphism $\varphi$. Suppose such a set $P$ in $\gmodel$ and suppose that $\varphi(P)$ is representative in $\gmodel'$.  Suppose that  for some $p,p' \in P$, $p \simeq p'$. Since $\varphi$ is an isomorphism it follows that $\varphi(p) \simeq \varphi(p')$ in $\gmodel'$ (lemma \lref{mappingspreserveequivalence}) and then since $\varphi(P)$ is assumed to be representative 
...
In a relational schema, attributes typed nby specific domains of values but for our purpose we can assume that each attribute is $v$-valued where $v$ is our type of universals for domains play no role, for the time being at least, in the theory with which we concern ourselves here.
...
We described the first-cut transformation of a model $\gmodel$ in section \lref{rkaconstruction}.
