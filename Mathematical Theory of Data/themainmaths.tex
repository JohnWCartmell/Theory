...
$\varphi^{-1}: \gmodel' \morph \gmodel$ is an isomorphism of ER models and
for each entity type $a$ of $\gmodel$, $\varphi^{-1}(\pk[\gmodel']{\varphi(a)}\simeq\pk{a}$.
\end{lemma}
\begin{proof}
$\varphi^{-1}$ is an isomorphism iff $\varphi$ is an isomorphism because (i) - (iv) are symetric in $\varphi$ and $varphi^{-1}$.

$\varphi^{-1}(\pk[\gmodel']{\varphi(a)}\simeq\pk{a}$ because
\begin{align*}
\varphi^{-1}(\pk[\gmodel']{\varphi(a)}&=G(F(x_1)) & & \\
                                      &=\pk{\varphi^{-1}(\varphi(a))} & & \mbox{since $\varphi{-1}$ is an ER model mapping }\\
