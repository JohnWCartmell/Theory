...
\end{proof}
\begin{definition}
An ER model $\gmodel$ is \term{restriction-free} iff for all entity types $a$ of $\gmodel$ and for all edges $y$ with domain $a$, if there exists paths $x_1,...x_n$   with domain $a$ in $\gmodel$,  and for some $j$, $1 \leq j \leq n$, $x_j \not\simeq \tuple{}$ and in every defining instance $E$,  
...
\begin{observation}
If $\gmodel$ is a restriction-free  ER model with an edge $y$ then there does not exist\commentary{This still isn't correct because we might want ide-mpotent $y$ less than idempotent $x$. CONSIDER FURTHER}
a path  $x$ which is not equivalent to the empty path and for which $y < x$.
\end{observation}
