...
A functional dependency \sfd{x_1,...x_n}{y} is said to be a \term{leaf functional dependency} iff
 the codomain of each $x_i$, $1 \leq i \leq n$, is $\veee$ and the codomain of $y$ is $\veee$.
\end{definition}
...
If $\gmodel$ is an ER model which is well-formulated, has the fd factoring property and is free of non-identifying referential attributes
...
First note that if $\set{\xn}$ is a mono-source we are done because then in every defining instance $E$ each $E_{x_i}$ is total
...
$\set{x_1,...x_n} \morph y$ is a leaf functional dependency in $\gmodel$. Since $\gmodel$ has the fd factoring propery then either $\set{\xn}$ is a mono-source 
and we are done or else these exists  paths $x'_1,...x'_{n'}$ 
such that 
...
$\chigmodel$ is leaf-restriction free by lemma \lref{wellformulatedandfreeofreferentialattributesimpliesleafrestrictionfree}  since $\gmodel$ is assumed
to be leaf-restriction free and  to be well-formulated.
