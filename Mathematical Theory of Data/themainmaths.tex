...
\begin{oldtt}
iff for all attributes $x:a \morph \veee$ 
such that there is a 
\oldt{relationship-like} \newt{non-empty} path $r:a \morph b$ and a \newt{non-empty} path $p:b \morph \veee$ such that
...
or else there is a non-empty \reinstatet{relationship-like} path $r$ in $\gmodel$
...
If the model $\gmodel$ is assumed to be reduced without caveat then $r$ must be a path of length 1 since and to be a mono-source\commentary{Feel that I need to spell this out a bit more} since $w$ is not equivalent to some primary key of $d$.
Since $r$ is a mono-source then $x_1,...x_n$ is a mono source, 
since $\msfd{x_1,...x_n}{r}$ \newt{by lemma \ref{fdrightmonosourceimpliesfdleftmonosource}}, as required and we have established BCNF.
