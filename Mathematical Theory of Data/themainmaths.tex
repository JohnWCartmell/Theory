...
From lemma \lref{primarykeypathequivalentsthatfactorthroughidentifyinglike} it follows that $w \simeq q_i$, for some
$i$, $1 \leq i \leq n$.  
Therefore
\begin{align*}
...
\begin{definition} 
An ER model $\gmodel$ is \term{reduced without caveat} \begin{newtt}
...
or else there is a non-empty \reinstatet{relationship-like} path $r_1 \circ r_2 ... \circ r_m$ 
...
\begin{enumerate}[(i)]
\item $r_2 \circ ... \circ r_n \circ \retract(p)$ is a primary key path in $\gmodel$ and by observation \lref{pathinjectionobservation}, because $\retract(r_2 \circ ... \circ r_n \circ x) = r_2 \circ ... \circ r_n \circ \retract(p)$ that $r_2 \circ ... \circ r_n \circ x$ is a primary key path in $\chigmodel$, or 
...
preserves mono-sources (lemma \lref{isospreserveandreflectmonosources}) and preserves the property of a path not being relationship-like because it reflects the relationship-like property of paths(lemma \ref{isospreserveandreflectidentifyinglike}), \commentary{Seems convincing but a bit of work to do done bridging the gap to the two lemmas.}, or
...
Since $\gmodel$ is well-formulated  and since (\ref{intransitivefd}) is a leaf intransitive functional dependency  then either $\set{x'_1,...x'_{n'}}$ is a mono-source and thus by lemma \ref{fdrightmonosourceimpliesfdleftmonosource}
...
then the transformed model $\chigmodel$ is a  relational ER model that is well-formulated, \newt{reduced}, leaf-restriction free
...
$\chigmodel$ is \newt{reduced by lemma \lref{reducedcarrythroughlemma} }  because $\gmodel$ is \newt{reduced}.
...
\section{Late arriving lemmas from 26th 27th June 2020}
