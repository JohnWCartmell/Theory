...
or else for some entity type $d$ there is a non-empty \oldt{relationship-like} path $p:a \rightarrow d$ such that $\{x_1,...x_n\} \morph p$ is a functional dependency in $\gmodel$
...
there  is a \oldt{relationship-like} path $p:a \rightarrow d$ such
that 
...
or else there is an entity type $d$ and there is a \oldt{relationship-like} path $p:a \rightarrow d$ such that $\{x_1,...x_n\} \morph p$
...
or else there is a non-empty \oldt{relationship-like} path $r$ in $\gmodel$
and a path $w$ of $\gmodel$ that is not  equivalent to a primary key path such that \commentary{\newtt{18th June 2020}One other possibility where $r$ factors into a monos-ource. From here I cannot make the proof go through. \newt{NOT PROVEN.}}
\begin{equation}
