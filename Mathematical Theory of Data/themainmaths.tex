...
\begin{lemma}
\llabel{keyreflectionuptoeqivalence} \commentary{not required?}
...
In section \lref{definitionofrelational} we characterise those ER models which correspond to relational schemas and elsewhere\footnote{TBD} those that correspond to hierarchical schemas. 
...
$\setsuchthat{E_e}{e \in (i \backslash \set{f}) \cup \setsuchthat{f \circ g}{g \in j}}$
is jointly injective. Because of (\lref{eequivalentsets}) this follows by applying lemma \lref{jointlyinjectivecomposition} with $I$ being $\setsuchthat{E_e}{e \in i}$,
...
Further if (\lref{sourcerefincd}) is a leaf primary mono source then (\lref{targetrefincd}) is a leaf primary mono-source.
\end{lemma}
\begin{proof}
To show that (\lref{targetrefincd}) is a referential inclusion dependency
...
$E'_{\fntuple}=E_{\fntuple}$ and $E'_{\qntuple}=E_{\qntuple}$ and because \commentary{CHECK THIS}
(\lref{sourcerefincd}) is a referential inclusion dependency in $\gmodel$.

That if  (\lref{sourcerefincd}) is a leaf primary mono source then (\lref{targetrefincd}) is a leaf primary mono-source also follows from lemma \lref{mappingspreservemonosources}.\commentary{CHECK THIS}
\end{proof}
...
Further, (\lref{targetincd}) is a leaf primary referential inclusion dependency  iff (\lref{sourceincd}) is a leaf primary referential inclusion dependency.
\end{lemma}
\begin{proof}
That (\lref{sourceincd}) is an inclusion dependency in $\gmodel$ implies 
(\lref{targetincd}) is an inclusion dependency in $\gmodel'$ is established in lemma
\lref{mappingspreservereferentialincds} which also
establishes that if (\lref{sourceincd}) is a leaf primary referential inclusion dependency
then so to is (\lref{targetincd}). To establish the reverse implication use lemma
...
(\lref{targetincd}) is a leaf primary referential inclusion dependency that is
...
are paths in $\gmodel$ and $p \simeq p'$ then $p$ represents (\lref{incdequivalence1})
iff $p'$ represents (\lref{incdequivalence2}). 
\end{lemma}
\begin{proof}
tbd
\end{proof}
...
\end{equation}
is a referential inclusion dependency in $\gmodel'$
then (\lref{isosourceincd}) is represented in $\gmodel$ by a path $p$
iff (\lref{isotargetincd}) is represented in $\gmodel'$ by the path $\varphi(p)$.
\end{lemma}
\begin{proof}
TBD
\end{proof}
...
\end{equation}
in $\gmodel'$.
Further if (\lref{carrythrusourcefd}) is trivial then (\lref{carrythrutargetfd}) is trivial.
\end{lemma}
\begin{proof}
Assume a functional dependency (\lref{carrythrusourcefd})  in $\gmodel$. To show that (\lref{carrythrutargetfd}) is a functional dependency in $\gmodel'$
...
$E_{\tuple{\xn}} = E'_{\tuple{\phixn}}$ and $E_y = E'_{\phiy}$. From the assumption that (\lref{carrythrusourcefd})
...
meets the conditions (\lref{fdcarrythrucond1}) and (\lref{fdcarrythrucond2}) that
...
A functional dependency \sfd{x_1,...x_n}{y} is said to be a \term{leaf functional dependency} iff
 the codomain of each $x_i$, $1 \leq i \leq n$, is $\veee$ \commentary{and the codomain of $y$ is $\veee$ I would have thought.}
\end{definition}
...
\end{equation}
in $\gmodel'$.
Further(\lref{sourcefd}) is  leaf  iff (\lref{targetfd}) is a leaf.

\end{lemma}
\begin{proof}
\vspace{0.5cm}
That (\lref{sourcefd}) is a functional dependency implies
(\lref{targetfd}) is a functional dependency has been shown in lemma \lref{mappingspreservefds}. That (\lref{targetfd}) is a functional dependency implies
(\lref{sourcefd}) is a functional dependency follows by using lemma 
 \lref{mappingspreservefds}
on the inverse mapping $\varphi^{-1}$ and by using lemma  \lref{fdequivalence}.

(\lref{sourcefd}) is  leaf  iff (\lref{targetfd}) is a leaf 
...
\end{equation}
in $\gmodel'$.
Further (\lref{multiplesourcefd}) is reversible iff (\lref{multipletargetfd}) is reversible.
\end{corollary}
...
\begin{enumerate}
\item (\lref{transitivesourcefd}) is  transitive iff (\lref{transitivetargetfd}) is transitive, 
\item (\lref{transitivesourcefd}) is  intransitive iff (\lref{transitivetargetfd}) is intransitive,

\end{enumerate}
\end{lemma}
\begin{proof}
\vspace{0.5cm}
(\lref{transitivesourcefd}) is transitive implies (\lref{transitivetargetfd}) 
is trasitive because if (\lref{transitivesourcefd}) is transitive is transitive then
...
the function dependency (\lref{transitivetargetfd}) is transitive.

That (\lref{transitivesourcefd}) is  intransitive iff (\lref{transitivetargetfd}) follows on directly because an instransitive functional depdency is a functional depdency that is not transitive.

Finally, a path $p$ represents (\lref{sourcefd}) in $\gmodel$ iff $\varphi(p)$ represents (\lref{targetfd}) in $\gmodel'$
because
...
(\lref{representationsourcefd}) is represented in $\gmodel$ iff
(\lref{representationtargetfd}) is represented in $\gmodel'$.
\end{lemma}
\begin{proof}
TBD
\end{proof}
...
\begin{align*}
E_y &= E_{\xntuple} \circ E_{\qntuple}^{-1} \circ E_w  && \text{ (\lref{fundependency}), above,}\\
    &= E_p \circ E_w                                   && \text{by (\lref{incdependency}),}\\
...
We shall show that with these definitions if $\gmodel$ is well-formulated, simple, $\chit$-suitable and has the fd factoring property \commentary{check this summary}
...
\begin{lemma}
\llabel{wellformulatedandfreeofreferentialattributesimpliesleafrestrictionfree}
...
Since $\gmodel$ is well-formulated  and there is a leaf intransitive\commentary{why is it intransitive?} functional dependency $\set{x_1,...x_n} \morph y$ then either $\set{\xn}$ is a mono-source and thus $E_{\xntuple}$ is injective and total in every defining instance $E$  in which case $y \simeq x_j$  and we are done \commentary{why?} or else the functional dependency $\set{x_1,...x_n} \morph y$   is in part represented by  an entity type $d$ and a primary mono-source $\qntuple$ with respect to\commentary{sourced at $d$?} $d$, such that $\incd{a}{x_1,...x_n}{d}{q_1,...q_n}$ but this cannot be the case because if it were then by lemma \lref{inclusiondependencyrestrictionlemma}
$$E_p \circ E_{q_j} = \bigwedge\limits_{\genfrac{}{}{0pt}{}{1 \leq i \leq n}{ i \neq j}}{\overline{E_{x_i}}}\circ E_{x_j}$$
...
Since $\gmodel$ is free of non-identifying referential attributes then $y$ is identifying and so $E_y$ is total in all instances $E$ and therefore 
 from (\lref{restriction}) it follows that in any instance $E$, $E_{x_i}$ is total and therefore that $E_y =E_{x_j}$, that is 
$y \simeq x_j$. We have shown that from the assumption (\lref{restriction}) it follows that $y \simeq x_j$. Thus we have shown that $\gmodel$ is leaf-restriction free.
\end{proof}
...
\begin{lemma}
\llabel{wellformulatedandfreeofreferentialattributesimplieschileafrestrictionfree}
If $\gmodel$ is $\chit$-suitable ER model which is well-formulated and has no referential attributes 
then $\chigmodel$ is leaf-restriction free.
\end{lemma}
\begin{proof}
Uses lemma  \lref{wellformulatedandfreeofreferentialattributesimpliesleafrestrictionfree} and lemma \lref{restrictioncarrythroughlemma}. 
\vspace{0.5cm}
tbd
\end{proof}
...
$\chigmodel$ is well-formulated by lemma \lref{wfcarrythroughlemma}.

$\chigmodel$ has the fd factoring property by lemma \lref{fdfactoringcarrythroughlemma}.

$\chigmodel$ is leaf-restriction free by lemma \lref{wellformulatedandfreeofreferentialattributesimpliesleafrestrictionfree}.

It follows directly from definitions that $\gmodel$ is simple since it is free of non-identifying referential attributes.
By lemma \lref{simplecarrythroughlemma} it follows that $\chigmodel$ is simple .
