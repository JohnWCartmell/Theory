...
\begin{definition}
In an  ER model $\gmodel$, if $a$ and $b$ are entity types and if $\set{\qntuple}$ is the set of primary key paths sourced at $b$, if $r: a \morph b$ is a relationship the $r$ is said to be a \term{pseudo-restriction}\commentary{Find a better term here.}
...
\end{lemma}
\begin{proof}
Uses lemma  \ref{wellformulatedandfreeofreferentialattributesimpliesleafrestrictionfree}.
\vspace{0.5cm}
tbd
\end{proof}
...
$\chigmodel$ is a  relational ER model by lemma \ref{}.
...
\begin{enumerate}[(i)]
\item  if $\gmodel$ is 
free of non-identifying referential attributes
then  $\chigmodel$ is in third normal form, 
\item 
if it is free of any referential attributes at all then  $\chigmodel$ is in BCNF.
\end{enumerate}
\end{theorem}
\begin{proof}
