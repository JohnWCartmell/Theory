...
We have defined isomorpisms of ER models to preserve and reflect primary key paths upto equivalence of
paths in the sense given by these lemmas:

keypreservationuptoeqivalence
\begin{lemma}
\label{keypreservationuptoeqivalence}
If $\gmodel$ and $\gmodel'$ are ER models and if $\varphi$ is an ER model isomorphism $\varphi: \gmodel \morph \gmodel'$ then whenever $p':\varphi(a) \morph \veee$ is a primary key path in $\gmodel'$ then there exists a primary key path $p:a \morph \veee$ in $\gmodel$ such that $\varphi(p) \simeq p'$ in $\gmodel'$.
\end{lemma}

keyreflectionuptoeqivalence
\begin{lemma}
\label{keyreflectionuptoeqivalence}
...
certainisospreserveidentifyinglike % KEEP AS IS
\begin{lemma}
...
is identifying-like in $\gmodel$ let $q : \varphi(b) \morph \veee$ be a primary key path in $\gmodel'$. By lemma 2.33 there exists a primary key path $p : b \morph \veee$ in $\gmodel$ such that $\varphi(p) \simeq q$. Because $t$ is assumed to be identifying-like then there exiosts a primary key path $p': a  \morph  \veee$ in $\gmodel$ such that $t \circ p \simeq p'$. By lemma 2.32 there exists a primary key path $q' : \varphi(a)  \morph  \veee$ in $\gmodel'$
...
\varphi(t) \circ q  & \simeq \varphi(t) \circ \varphi(p)       && \mbox{ since $q \simeq \varphi(p)$}    \\
                    ...
                    & \simeq \varphi(p')                        && \mbox{ from choice of $p'$}           \\
...
\end{lemma}
\begin{proof}
Use lemmas \ref{identifyinglikeequivalence} and \ref{certainisospreserveidentifyinglike}.
\end{proof}
...
\end{proof}
\begin{definition}
An ER model $\gmodel$ is \term{restriction-free} iff for all distint nodes $a$, $b$ of $\gmodel$ and for all edges $y: a \morph b$, if there exist non-empty paths $x_1,...x_n$   with domain $a$ in $\gmodel$,  and for some $j$, $1 \leq j \leq n$, $x_j \not\simeq \tuple{}$ and in every defining instance $E$,  
