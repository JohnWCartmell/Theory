...
\begin{observation}\commentary{Question over whether this is required.}
\label{factoringprimarykeypath}
If $Q \in \bar{I_a}$ and if $p$ is a primary key path at $a$ then for some node $b$ there exists $q \in Q$, $q: a \morph b$\commentary{q may be empty, may have $p \in Q$}, and there exists 
...
\begin{lemma}{Question over whether this is required.}
...
\commentary{It is the case that a leaf primary mono-source is equivalent to $\pk{a}$??}
...
$J \in \in \bar(I_a)$ such that $Q \simeq J$. \commentary{Work on this. LEMMA needed?}
\end{proof}
...
(lemma \ref{mappingrespectsequivalence}).\commentary{say more?}
...
anisoreflectsprimariesiffitsinversepreservesthem
\begin{lemma}
...
That if  (\ref{sourcerefincd}) is a leaf primary mono source then (\ref{targetrefincd}) is a leaf primary mono-source also follows from lemma \ref{mappingspreservemonosources}.\commentary{check this}
\end{proof}
...
is identifying-like.\commentary{A relationship-like path, as a relationship i.e. a path of length 1,  can be navigated in SQL without use of intermediate tables. Paths which have length greater than one and are not relationship-like need be navigated in SQL using intermediate tables.}
\end{definition}
...
$\varphi: \gmodel  \morph \gmodel'$ then $\varphi$ is said to be a \term{key-realising} isomorphism iff
...
certainisomorphismspreservewellformulatedness
\begin{lemma}
...
\commentary{Spell this out sometime??}
}
\item{ 
...
\end{proof}
\begin{definition}
An ER model $\gmodel$ is \term{restriction-free} iff for all entity types $a$ of $\gmodel$ and for all edges $y$ with domain $a$, if there exists paths $x_1,...x_n$   with domain $a$ in $\gmodel$,  and for some $j$, $1 \leq j \leq n$, in every defining instance $E$,  
...
a primary key path. Assume the contrary that $w$ is a primary key path. From lemma 
...
Hence because $\gmodel$ is leaf \commentary{check leaf} restriction-free we deduce that $y \simeq x_i$ 
\commentary{or $x_i$ is empty and $y \simeq x_j$ ???} and this contradicts the assumption that the functional dependency
...
\end{lemma}
\begin{proof}
Use lemma \ref{isospreserveandreflectfds} and also lemma \ref{isospreserveandreflecttransitivefds}.
To show that $\gmodel'$ has the fd factoring property if $\gmodel$ does assume $\gmodel$ has the fd factoring property and assume $\fd{\xnprimetuple}{z'}$ in $\gmodel'$.
By lemma \ref{????} it follows that $\fd{\varphi^{-1}(x'_1),...\varphi^{-1}(x'_n)}{\varphi^{-1}(z')}$
in $\gmodel$. Since $\gmodel$ is assumed to have the fd factoring property either $\set{\varphi^{-1}(x'_1),...\varphi^{-1}(x'_n)}$ is a mono-source in $\gmodel$ in which case $\set{\xntuple}$ is a mono-source in 
$\gmodel'$ by lemma \ref{???} and we are done or else there exists paths $\yn$ in $\gmodel$ such that
$\fd{\varphi^{-1}(x'_1),...\varphi^{-1}(x'_n)}{\yn}$ and 
an intransitive fd $\fd{\yn}{\varphi^{-1}(z')}$ in $\gmodel$.  From this is follows by lemma 
\ref{???} that $\fd{\xnprimetuple}{\varphi(y_1),...\varphi(y_n)}$ in $\gmodel'$
and such that $\fd{\varphi(y_1),...\varphi(y_n)}{z'}$ is an intransitive fd in $\gmodel'$. We have shown that $\gmodel'$ has the fd factoring property.

To show that $\gmodel$ has the fd factoring property if $\gmodel'$ does assume $\gmodel$ has the fd factoring property and assume $\fd{\xn}{z}$ in $\gmodel$. Then $\fd{\phixn}{\varphi(z)}$ in $\gmodel'$. 
Since $\gmodel'$ is assumed to have the fd factoring property then either $\set{\phixn}$ is a mono-source
in $\gmodel'$ in which case by lemma{???} $\set{\xn}$ is a mono-source in $\gmodel$ and we are done or else
there exists $\set{y_1,...y_n}$ such that
$\fd{\phixn}{\yn}$ is an non-trivial fd in $\gmodel'$ and $\fd{\yn}{\varphi(z)}$ is an intransitive fd
in $\gmodel'$ from which it follows that 
$\fd{\xn}{\varphi^{-1}(y_1),...\varphi^{-1}(y_n)}$ is an non-trivial fd in $\gmodel$ and $\fd{\varphi^{-1}(y_1),...\varphi^{-1}(y_n)}{z}$ is an intransitive fd
in $\gmodel$, as required.
\end{proof}
...
$\qntuple$ is a \commentary{leaf?} ??? primary mono-source at $b$ in $\gmodel$, where $q_i:b \morph \veee$, for each $i$, $1 \leq i \leq n$  then there exists attributes $x_i:a \morph v$ in $\gmodel$ such that $\incd{a}{\xntuple}{b}{\qntuple}$ is an inclusion dependency and this inclusion dependency is represented by the relationship $R$. 
...
\begin{definition}
In an  ER model $\gmodel$, if $a$ and $b$ are entity types and if $\set{\qntuple}$ is the set of primary key paths sourced at $b$, if $r: a \morph b$ is a relationship the $r$ is siad to be a \term{pseudo-restriction}\commentary{Find a better term here.}
...
\begin{lemma}
\label{relationshiplikecarrythroughlemma}\commentary{nonsense - it may reflect primaries and therefore reflect relationship-like which may be enough. Work forward from mappings and isomorphisms which reflect primaries and see where we get.}
