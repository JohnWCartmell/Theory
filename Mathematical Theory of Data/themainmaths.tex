...
If $\gmodel$ is an ER model then it can be extended to a model, denoted  $\gmodel + \qq{\underline{PK}}$
which is primarily keyed. If $\gmodel$ satisfies the minimality condition then the
extended model $\gmodel + \qq{\underline{PK}}$ satisfies the  condition that each set $I_a$ has no proper subsets which are mono-sources. 
There is an isomorphism from $\gmodel$ to $\gmodel + \qq{\underline{PK}}$.
\end{lemma}
\begin{proof}
Let $P$ be the set of primary key paths of $\gmodel$. We define the ER model $\gmodel + \qq{\underline{PK}}$ to be the
ER model   $\gmodel + \qq{PK}$ as described in section \ref{extensionbysetofpaths} but with identifying edges for any entity type $a$ being defined
by $I_a = \setsuchthat{\qq{p}}{p \in \pk{a}}$. 

To show that the ER model isomorphism $\inject: \gmodel \morph \gmodel + PK$ determines an ER model mapping  $\inject: \gmodel \morph \gmodel + \qq{\underline{PK}}$
we need show that it satisfies the condition that $\inject(\pk{a}) \simeq \pk[\gmodel + \qq{\underline{PK}}]{a}$. This is the case because for each primary key path $p \in \pk{a}$,
$\inject(p) \simeq \qq{p}$ in $\gmodel + \qq{PK}$ and each identifying edge of $\gmodel + \qq{\underline{PK}}$ arises in this way.

To show that the the inverse mapping $\retract: \gmodel + PK \morph \gmodel$ determines an ER model mapping  $\retract: \gmodel + \qq{\underline{PK}} \morph \gmodel$ 
we need show that for each entity type $a$, $\retract(\pk[\gmodel + \qq{\underline{PK}}]{a}) \simeq \pk{a}$. 
This is the case because each  $e \in \pk[\gmodel + \qq{\underline{PK}}]{a}$ there is a primary key path $p \in \pk{a}$ 
such that $e = \qq{p}$ and so $\retract(e) \simeq p$ in $\gmodel$ and to each primary key path $p \in \pk{a}$ there is an identifying edge $\qq{p} \in \pk[\gmodel + \qq{\underline{PK}}]{a}$
for which then  $\retract(\qq{p}) \simeq p$. 

The isomorphism from $\gmodel$ to $\gmodel + \qq{PK}$ thus determines an isomorphism from $\gmodel$ to $\gmodel + \qq{\underline{PK}}$. 
...
entity type $a$, by attributes,  one for each path in $\gmodel$ of the \commentary{Need define identifying edges}
form $r  \circ pk$ where $r$ is a relationship and $pk$ is a primary key path. 
...
equivalent attribute-like paths wrt the equivalence relation $\simeq$ of path equivalence. This improved transformation we can denote as $\firstcutimproved$.

This consideration leads  to this definition:
\begin{definition}
A path $r=r_0 \circ r_1 \circ r_2... \circ r_n$, for some $n$, $n \geq 0$, in an ER model $\gmodel$ is 
...
or else $s_1\ circ ... \circ s_n$ is non attribute-like and $r \circ q \simeq s_1\ circ ... \circ s_n$ then say that relationship $r$ \term{directly depends} on relationship $s_i$,
...
We exclude models $\gmodel$ that have boolean-worth relationships from the $\gmodel$ for which $\chi$ is defined but not because a relational impelmentation cannot be given but because there is a choice of representations: 
\begin{lemma}
...
\tbd
\end{proof}
We could easily include boolean-worth representations to entity types having a single \highlight{primary key attribute}\commentary{check} because then no choice is required.
...
If these conditions are met: that attribute-like paths have at most one equivalent non attribute-like path, that implied dependencies are non-circular and there are no  \newt{boolean-worth} relationships, then we say that $\gmodel$ is \term{$\chit$-suitable} and in these instances we can show that 
...
\begin{cases}
   ...
   I_\gmodel(s_i)     & \mbox{if $s_i$ is a non-attribute-like path and is equivalent to  $r \circ q_i$ in $\gmodel$ }  \\
