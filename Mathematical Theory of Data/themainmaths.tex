...
\item{a distinguished node $\veee$ for which there are no outgoing edges and which represents the type of universals, 
 the nodes of $\gmodel_S$ other than $\veee$ we say are entity types and the set of entity types we denote by $\et[\gmodel]$,} 
\commentary{we only use $\et[\gmodel]$ in proof of lemma \ref{ identifyingsetdeduction} and in commentary that follows it, nowhere else.}
...
that mapps entity types to entity tyoes and the node $\veee$ of $\gmodel$ to the distingished node
...
keyreflectionuptoeqivalence
\begin{lemma}
\label{keyreflectionuptoeqivalence} \commentary{not required?}
...
identifyinglikeequivalence
\begin{lemma}
\label{identifyinglikeequivalence}
...
\begin{observation}
\label{keypreservationuptoequivalence}
...
\begin{observation}
\label{keysurjectionuptoequivalence}
...
mappingspreserveidentifyinglike 
\begin{lemma}
\label{mappingspreserveidentifyinglike} 
...
isospreserveandreflectidentifyinglike
\begin{lemma}
\label{isospreserveandreflectidentifyinglike}
...
isosssurjectiveonrelationshipsaresurjectiveonidentifyinglike
\begin{lemma}
\label{isossurjectiveonrelationshipsaresurjectiveonidentifyinglike}
...
relationshippreservingmappingspreserverelationshiplike:
\begin{lemma}
\label{relationshippreservingmappingspreserverelationshiplike}\commentary{not sure that this is required}
...
isossurjectiveonrelationshipsaresurjectiveonrelationshiplike
\begin{lemma}
\label{isossurjectiveonrelationshipsaresurjectiveonrelationshiplike}
...
\end{proof}
\begin{definition}
An ER model $\gmodel$ is \term{restriction free} iff for all distinct nodes $a$, $b$ of $\gmodel$ and for all edges $y: a \morph b$, if there exist non-empty paths $x_1,...x_n$   with domain $a$ in $\gmodel$,  and for some $j$, $1 \leq j \leq n$, $x_j \not\simeq \tuple{}$ and in every defining instance $E$,  
...
chimodelisrelational
\begin{lemma}
\label{chimodelisrelational}
...
(lemma \ref{isossurjectiveonrelationshipsaresurjectiveonrelationshiplike}) and a path $\retract(p)$ in $\gmodel$ \commentary{check this} such
...
wellformulatedandfreeofreferentialattributesimpliesleafrestrictionfree
\begin{lemma}
\label{wellformulatedandfreeofreferentialattributesimpliesleafrestrictionfree}
...
Now it follows from \ref{restrictioninchimodel} that, in $\chigmodel$, $\qq{r \circ q} < x'_j$. Therefore \commentary{what else to add here} 
...
wellformulatedandfreeofreferentialattributesimplieschileafrestrictionfree
\begin{lemma}
\label{wellformulatedandfreeofreferentialattributesimplieschileafrestrictionfree}
