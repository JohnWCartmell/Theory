...
Suppose in every instance $D$ of a requirement $\reqtc$ for $\catc$, the function $D(u)$ is a bijection. In this case  
...
To meet the goodness criteria, $S$ can be extended to a sketch $S'$ having an additional arrow $v:e \morph e$ to represent this functional
dependency and defining $D(v)=D(u)^{-1}$ in each instance $D \in \reqtc$. 
With these definitions then commutivity constraints $D(u) \circ D(v)=id_{D(e}$ and $D(v) \circ D(u)=id_{D(e}$ hold
in all the instances $D'$ of the extended $\reqtcp$ and there for path equivalences $u \circ v=id_e$ and $v \circ u =id_e$
must also be added to the sketch $S'$ if the presented category $\reqtcp$ to be maximally constrained to the
requirement $\reqtcp$.

With $S'$ and $\catcp$ so defined then all commutivity constraints, all functional dependencies and all referential inclusion dependencies present in the requirement  $\reqtcp$ will be represented in $\catcp$ however this does not mean that
...
$D(u)$ is the square of some other bijection $w_D$ on set $e$. In this case in each instance $D \in \reqtcp$ there is a unique
function $w_D: D(e) \morph D(e)$ such that $w_D \circ w_D = D(u)$. If this is the case then $\catcp$ is not maximally constrained
to $\reqtcp$ for a morphism $w$ can be added to the sketch such that $w \circ w = u$ and the extended theory is a
better fit to the requirement than $\catcp$. In fact $u$ and $v$ will then be dropped from the sketch as they will have become redundant.
...
such a data representation would not necessarily fail any of the classic goodness criteria, it is counter to the intuition we have of good data design. It seems that
a definition of the ilk of the maximal constrainedness definition might better formalise this intuition.
...
data in a preliminary design of such say like this

then it might be.

It also might be as \textit{after we take away detail} from a complete speification to achieve an abstraction. Thus we might find
directed graphs within a completed data specifications wherever we can find this diagram:
$$
\begin{array}{c p{0.25cm} c  p{0.25cm} c }
...
I mention this because it is a point enthusiastically made by Bachman\footnote{though using different language to ours} in his 1969 paper \textit{Data Structure Diagrams} \cite{Bachman1969}. 
