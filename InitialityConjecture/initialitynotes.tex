...
They are equivalent concepts\footnote{Though I haven't proved this lemma in my thesis it is entirely straightforward and I did write out the proof of a  corresponding lemma in regard to single-sorted algebraic theories in my Msc dissertation.} 
\end{point}
...
$\ccat[C]$ then we can define a local product object $\crossx{B_1}{B_2}{A}$ such that $B_1 \base \crossx{B_1}{B_2}{A}2$ in $\ccat[C]$
by
\begin{equation*}
\crossx{B_1}{B_2}{A} =p_{B_1}^*B_2
\end{equation*}
if $X$ is an object of \catc and if $h_1:X \morph B_1$ and $h2:X \morph B_2$ are morphisms in \catc such that
$h_1 \circ p_{B_1}= h_2 \circ p_{B_2}$ then define $\tuple{h_1,h_2}=s(h_2)\circ q(h1,p_{B_1}^*B_2$.
...
\footnote{Generally we would be thinking of a category with finite products including terminal object, here, though as they say the categories  not all products need be available in the category for there to be an internal monoid.}.
...
the models are monoids internal to these other cwfs. In other words the generalised algebraic theory of xyz's is something that has xyz's as models in Fam and has internal xyz's as models in other categories. Nonetheless there is a theory of internal xyz's -- its models in Fam are internal xyz's i.e. they are contextual categories (or cwfs) along with internal xyz's. When the theory of internal xyz's is interpreted in an arbitrary contextual category (or cwf) then what results is an internal internal xyz! This sounds a bit crazy but it isn't -- there are after all categories internal to other categories and it isn't much of a stretch to suppose these internal categories have internal xyz's inside of them, so to speak. 
\end{notebox}
