...
g \circ p_C) ^* C &= \duplesi ^* C  \\
                  &= (\duplesnlessone \circ {p_{B_{n-1},B_i}})^* C\\
									&=\duplesnlessone ^* B_n
\end{align*}
},  then if $s(g)=s_n$ then $\duplesn = \bigtuple{\duplesnlessone,g}_{\crossx{B_{n-1}}{C}{B_i}}$\,.
\end{lemma}
\begin{proof}
From the definition of $\llangle\rrangle_{p_{B_{n-1},B_i},C}$, it suffices  to show that 
\begin{equation}
\duplesn \circ p_{B_{n-1},B_i} = \duplesnlessone
\end{equation}
and
\begin{equation}
...
\note DEFINITION OF $\duplesn$ HERE xxxxxx 
