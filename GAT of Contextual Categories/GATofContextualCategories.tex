...
\noindent If $f:x \longrightarrow y_p$, $y_p \base y$, and $g:y_p \longrightarrow y$ in a contextual category \cat{C} then if $g$ is a section i.e. if $g \circ p_y = id_{y_p}$ then  $f \sub g : x \longrightarrow f \sub y$ and 
$f \sub g$ is a section.
\end{lemma}
\begin{proof}
If $g$ is a section then the codomain $(f \circ g \circ p_y) \sub y$ of $f \sub g$ simplifies to $f \sub y$.
To show that $f \sub g$ is a section we
need to show $s(f \circ g) \circ p_{f \sub y} = id_x$. This follows from axiom s1.
\end{proof}
...
\noindent
Axiom s1 holds because of lemma \ref{lemmastarsection} and axiom \ref{delta1}.
\noindent
