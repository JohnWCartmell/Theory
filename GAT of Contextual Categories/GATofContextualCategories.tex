...
we introduce two new operators. The first is $\hash$ and applies to certain morphisms as follows: 
...
\begin{proof}
\begin{align*}
lhs &= s(f_1 \circ f_2 \circ g)     && \mbox{from defn. of extended $\hash$} \\
    &= s(f_1 \circ s(f_2 \circ g))  && \mbox{by lemma \ref{s4}} \\
		&= f_1 \hash f_2 \hash g    && \mbox{from defn. of extended $\hash$} \\
...
s(f) \circ q( f \circ p_y ,y)
 ...
 &= (f \hash \delta_y) \circ q(f,{p_y} \ssub y) \circ q(p_y,y)                                     && \mbox{by \ref{q5},} \\
 &= (f \hash \delta_y) \circ q(f \circ \delta_y \circ p_{{p_y} \ssub y},{p_y} \ssub y) \circ q(p_y,y) && \mbox{since $\delta_y$ is a section,} \\
 &= f \circ \delta_y \circ q(p_y,y)                                                             && \mbox{from \ref{hash2},} \\
...
s(f \circ q(g,z)) &= (f \circ q(g,z)) \hash \delta_z      && \mbox{from the  definition given for $s$,} \\
                  ...
                  &= f \hash \delta_{g \sub z}            && \mbox{from lemma \ref{xdelta3},} \\
