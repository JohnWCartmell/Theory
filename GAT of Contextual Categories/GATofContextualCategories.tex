...
\ncasar{p_{f \sub y_2}}{fy2}{x}
\ncbsar{p_{y_1}}{y1}{y_p}
\ncasar{p_{y_2}}{y2}{y_p}
\ncasar[0]{p_{{{p_{f \sub y_1}}\sub {f \sub y_2}}}}{TOP}{fy1}
\jcbarr{f}{x}{y_p}
\ncaarr{g}{y1}{y2}
\ncaarr[25]{q(f,y_1)}{fy1}{y1}
\ncaarr[25]{q(f,y_2)}{fy2}{y2}
\ncaarr[45]{q(f,y_1) \circ g}{fy1}{y2}
\ncaarr[15]{}{TOP}{fy2}
\naput[npos=0.3]{\footnotesize $q(p_{f \sub y_1},f \sub y_2)$}
\ncaarr[80]{s(q(f,y_1) \circ g)}{fy1}{TOP}
\psset{linestyle=dashed}
...
If $f:x \rightarrow y_p$ in a contextual category \cat{C}, then
\begin{enumerate} [(i)]
...
If $f:x \rightarrow y_p$ in a contextual category \cat{C},
...
\subsection{I.B delta-star system.}

\subsubsection{Definition}

A delta-star system is a tree category \cat(C) with an operations $\sub$ and
...
\noindent
Need to be able to prove axiom s2 
\begin{equation}
s(f) \circ q( f \circ p_y     ,y)=f
\end{equation}
\noindent
...
lhs &= f \sub \delta_y \circ q(f \circ p_y, y)  && \mbox{from defn of s} \\
  ...
  &= f \circ \delta_y \circ q(p_y,y)        && \mbox{from \ref{star1}} \\
...
lhs &= (f \circ q(g,z)) \sub \delta_z    && \mbox{from defn of s} \\
  &= f \sub (  q(g,z) \sub \delta_z)     &&\mbox{by \ref{star2}} \\
