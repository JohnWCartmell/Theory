...
\begin{lemma}
 If \cat{C} is a tree-structured category with operations, q, * and s satisfying
...
\begin{lemma} [lemma s7\footnote{implemented as rule 17}]
\label{s7}  
For all morphisms $f: x \rightarrow y$ ,
...
\end{array}
\end{displaymath}
\ncaarr{q(s(f),(p_{(f \circ p_y) \sub y} \circ p_x) \sub x)}{px*x }{CLEFT}
\jcbarr{f}{x}{y}

\ncbsar{p_{(f \circ p_y) \sub p_y }}{C}{D}
\ncasar{p_{p_x \sub x}}{px*x}{x}
\ncasar{}{D}{x}
\ncbsar{p_x}{x}{xp}

\ncarr[30]{x}{px*x }
\naput[npos=0.7]{\footnotesize $s(id_x)$}
\ncarr[-25]{x}{D}
\nbput[npos=0.7]{\footnotesize $s(f)$}
\jcbarr[-25]{s(p_{f \circ p_y} \sub y)}{D}{C}
%\nbput[npos=0.5]{\footnotesize $s(p_(f \circ p_y) \sub y)$}
\end{center}
\end{lemma}
\begin{proof}
\begin{align*}
...
\begin{lemma} [lemma sinjection]
\label{s4}  
If \cat{C} is a contextual category then for all morphisms $f: x \rightarrow y$ and $f':x \rightarrow y$ in \cat{C},
...
Use of the $\sub$ notation can be extended in two ways.
\subsubsection{Extending * to Morphisms}
If $f:x \rightarrow y_p$ in a contextual category \cat{C}, and if $y_p \base y_1$ and $y_p \base y_2$ and 
$g: y_1 \rightarrow y_2$ in \cat{C} such that $g \circ p_{y_2}=p_{y_1}$ then we can use the Voevodsky construction (or, equally, the pullback structure)
...
\begin{center}
\begin{displaymath}
\begin{array}{cccp{1.5cm}ccc}
 \Rnode{fy1}{f \sub y_1} &     & \Rnode{fy2}{f \sub y_2} && \Rnode{y1}{\ \ y_1} &       & \Rnode{y2}{\ \ y_2}\\ [1.2cm]
                         &\ \ \Rnode{x}{x}\ \ &                  &&                 &\ \ \Rnode{y_p}{y}\ \ &            
\end{array}
\end{displaymath}
\psset{labelsep=0.05}
\ncbsar{p_{f \sub y_1}}{fy1}{x}
\ncasar{}{fy2}{x}
\ncbsar{p_{y_1}}{y1}{y_p}
\ncasar{p_{y_2}}{y2}{y_p}
\jcbarr{f}{x}{y_p}
\ncaarr{g}{y1}{y2}
\ncaarr[35]{q(f,y_1)}{fy1}{y1}
\ncaarr[25]{q(f,y_2)}{fy2}{y2}
\ncaarr[55]{q(f,y_1) \circ g}{fy1}{y2}
\psset{linestyle=dashed}
\ncaarr{f\sub g}{fy1}{fy2}
\end{center}
...
f \sub g \circ p_{f \sub y_2}
    ...
    & = s(q(f,y_1) \circ g) \circ p_{(p_{f \sub y_1})\sub {f \sub y_2}} \circ p_{f \sub y_1}  &&\mbox{by \ref{q1}}\\
		...
		& = s(q(f,y_1) \circ g) \circ q(f,y_1) \circ p_{y_1} \circ p_{f \sub y_1}  &&\mbox{by \ref{q1}}\\
		& = s(q(f,y_1) \circ g) \circ q(f,y_1) \circ g \circ p_{y_2} \circ p_{f \sub y_1}  &&\mbox{since $g \circ p_{y_2}=p_{y_1}$}\\
		& = p_{f \sub y_1}                             &&\mbox{by axiom \ref{s1}}
\end{align*}
\noindent (This proof more long-winded than I feel it needs to be. Drop suffixes to p's and rely ion well-typed-ness would be simpler.No - woprse something not right.)
