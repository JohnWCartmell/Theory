...
\note
By a \term{tree-structured category} we mean (i) a category with a tree-structure defined on its objects such that the tree of objects has a unique root object and (ii) for every $x \base y$ in the tree of objects  a canonical morphism $p_y:y \rightarrow x$.  I shall shall say morphisms of this form  are \term{direct dependency morphisms} and they will
...
\note
The contextual category structure supplies us with pullbacks for any $\smorph$ morphism, \commentary{Need improve \highlight{smorph}}
...
\end{center}
\note
With respect to any pullback diagram, if
 \ \ 
$
\ccsquareoutline{0.9cm}{1.2cm}{f^*z}{z}{x}{y}
\ccsquareacross{q(f,z)}{f}
\ccsquaredown{p_{f \sub z}}{p_z}
$
is an extended (\highlight{??})pullback diagram \commentary{I need to make my mind up whether I use tuple for all pullbacks or just primitive pullbacks}
in \catcw  so that for all objects $w$ of \catc, and for all
...
then we use the notation $\tuple{h_1,h_2}_{x,y,f,z}$  for the unique morphism   
...
On such diagrams as shown here the $p$ labels on $\dmorph$ morphisms (inclusive of their indices) are entirely predictable  and so in diagrams that follow we may omit them.
