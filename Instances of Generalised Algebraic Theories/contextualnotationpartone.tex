...
\commentary{This is the metaGAT idenity but \highlight{do we need} to replace $x_p$ by any $w$, $w < x$?}
...
and
(ii) is a special case of lemma \lref{stardistributesonmorophismslemma}.
\end{proof}
\begin{lemma}
\llabel{sofplemma}
...
\begin{proof}
By induction on $i$. 
In the case of $i=1$ we have\commentary{\highlight{concision}: arrange so that induction starts from $n=0$ with nothing to prove.} 
\begin{align*}
