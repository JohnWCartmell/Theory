...
\begin{lemma}
If $n > 1$ \highlight{CHECK} and if $w$, $\xn$ and $z$ are objects of a contextual category \catcw such that $w \base x_1 \base ... \base x_n$ and $w \base z$
and if $f \in Sect(x_1)$ in \catcw then
\begin{enumerate}[(i)]
\item $f * (\yz)= \crossx{(f^*y)}{z}{w}$,
\item if $g \in Sect(z)$ then $f * (\yg)= \crossx{(f^*y)}{g}{w}$.
\end{enumerate}
as shown here:
\begin{displaymath}
