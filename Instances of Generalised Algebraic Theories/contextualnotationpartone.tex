...
\newcommand{\rhsdom}{f^*(g^*z_1)}
\newcommand{\rhscod}{f^*(g^*z_2)}
\newcommand{\rhs}{f^*(g^*h)}
...
By lemma \lref{stardistributesonobjects} $\lhsdom=\rhsdom$ and $\lhscod=\rhscod$
...
\\
\Rnode{rhsdom}{\rhsdom} &\Rnode{rhsarc}{} & \Rnode{rhscod}{\rhscod}\\[0.75cm]
...
\equality{lhsdom}{rhsdom}
\ncline [doubleline=true, nodesep=0.2cm, offsetB=-0.15cm]{lhscod}{rhscod}
\nccdar{lhsdom}{fgdom}
\nccdar{lhscod}{fgdom}
\nccdar{fhdom}{fgcod}
\nccdar{fhcod}{fgcod}
...
\ncarr{hdom}{hcod}
\alabel{\h} \\
and we need to prove that $\lhs=\rhs$.
\end{proof}
...
\end{proof}
\begin{lemma}
\llabel{sofpsubstitutionlemma}\commentary{Can probably simplify this to single substitution of sofp}\commentary{\highlight{why $a_{n+1}$?}}
...
\end{proof}
\begin{lemma}
\llabel{secondsofpsubstitutionlemma}\commentary{Can probably simplify this to single substitution of sofp}\commentary{\highlight{why $a_{n+1}$?}}
