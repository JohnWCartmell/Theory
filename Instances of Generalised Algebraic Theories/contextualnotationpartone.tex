...
%\newcommand{\extq}[2]{\qbar(#1,#2)}   %sadly I have previously used \extq for quine quote
...
We present the basic properties of the extended $p$, $q$ and $^*$ notation in the form of a  revised definition of contextual category. This revised definition is easily be shown to be  equivalent to the original definition\footnote{Note that (\ref{qq1}) to (\ref{qq5}) in thgis definition are extended forms of  (q1) to (q5) from the original definition.}.
...
\begin{lemma}
If $f:x \morph y$ in contextual category \catcw then $\extq{f}{y}=f$.
\end{lemma}
\begin{proof}
...
for all morphisms $f: x_1 \morph x_2$ and $g: y_1 \morph y_2$, \commentary{this lemma has neem more general -- as the next lemma is}
\begin{equation}
...
\lhs \circ \qfgh \circ p_{g^*z_2,\gdom} 
 ...
 &= \lhs \circ p_{\fghcod,f^*y_1} \circ q(f,y_1)&&\mbox{by (Q1) \highlight{plus a bit?},} \\
 &= p_{\fghdom,f^*y_1} \circ q(f,y_1)&&\mbox{by definition of $\lhs$,} \\
 &= q(f,g^*z_1) \circ p_{g^*z_1,y_1}&&\mbox{by (Q1) \highlight{plus a bit?},} \\
...
\crossx{x}{(\crossx{y}{z}{w})}{w} 
                   ...
                   &= (q(p_{x,y},y) \circ p_{y,w}) ^* z       && \mbox{by (Q4)\highlight{??},}                           \\
...
\end{lemma}
\begin{proof}
We can show (i) holds as follows: \commentary{Can I not just use \lref{stardistributesonobjects} to prove this this?}
\begin{align*}
...
\end{align*}
By a similar argument we can show that (ii) holds. \commentary{Will it really be similar -- might I need new lemmas such as a morphism \highlight{variant of Q4}? }\commentary{Can I use lemma \lref{stardistributesonmorphisms} for this part of the proof?}
\end{proof}
