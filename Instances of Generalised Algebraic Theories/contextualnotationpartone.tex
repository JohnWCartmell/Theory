...
We present the basic properties of the extended $p$, $q$ and $^*$ notation in the form of a  revised definition of contextual category. This revised definition is easily be shown to be  equivalent to the original definition.

A  \term{contextual category} is
\begin{itemize}
\item 
...
and the following additional structure\footnote{Note that (\ref{qq1}) to (\ref{qq5}) are extended forms of  (q1) to (q5) from the original definition.}
...
\ncarr{BL}{BR}\blabel{f}
\ncleftsection{BL}{TL}\alabel{f^*g}
\ncleftsection{BR}{TR}\alabel{g}
}
...
(f^*g)^*(f^*z)&= (f^*g)^*(q(f,y_2)^*z)  && \mbox{ by lemma \lref{extendedstarcoheres}} \\
              &= ((f^*g) \circ q(f,y_2))^*z && \mbox{ by (q4) \highlight{(Q4), surely}} \\
