...
% ########### --- FOR DISTRIBUTION --- ######################################
\renewcommand{\llabel}[1]{\label{#1}}   % For a distribution copy 
\renewcommand{\lref}[1]{\ref{#1}}       % redefine these three commands
...
\usepackage{geometry}
\geometry{a4paper, top=2.0cm, left=2.5cm, right=2.5cm, bottom=2.0cm} % I did previously have problem
                                                                     % with top margin. Turned out to be becacause I has
                                                                     % dvi2ps -t letter 
                                                                     % instead of dvi2ps -t A4
...
% workaround when used with Rnode for size of font used under the 
