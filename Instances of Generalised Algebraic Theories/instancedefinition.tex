...
\Ibar(r_A)
   ...
   &=\iI_{sort}(A)
   &&\mbox{by identity (a) of lemma \lref{secondsofpsubstitutionlemma}, as required.}
\end{align*} 
\item For an operator symbol $f$ the proof is similar to the above, but in this case using clause (b) of lemma \lref{secondsofpsubstitutionlemma}.
\end{enumerate}
\end{proof}
...
\begin{enumerate}[(i) ]
    ...
    \begin{enumerate}[(a)]
        ...
        \begin{enumerate}[(a)]
            ...
            \highlight{Assumption that $n \geq 1$!!!}\commentary{patch required to definitions maybe}
...
We have to show that the rule \gatdisplayrule{Q}{\Omega[\SUBsFORy{m}]=\Omega'[s'_1|y_1...s'_m|y_m]} is consistently intepreted by $\Ibar$
 from the assumption that each of the rules \gatdisplayrule{\yOmega{m}}{\Omega=\Omega'} and
\gatdisplayrule{Q}{s_j=s'_j \in \Omega\SUBsFORy{j-1}}, \foreachj, are consistently intepreted by $\Ibar$.
From these inductive hypotheses we know that
\begin{equation}
\label{SI1inductiveequation1}
\Ibar(r_\Omega)=\Ibar(r_{\Omega'})
\end{equation}
where $r_\Omega$ is the rule \gatdisplayrule{\yOmega{m}}{\isT{\Omega}} and $r_{\Omega'}$ is the rule \gatdisplayrule{\yOmega{m}}{\isT{\Omega'}} 
and that, \foreachj,
\begin{equation}
\label{SI1inductiveequation2}
\Ibar(r_{s_j})=\Ibar(r_{s'_j})
\end{equation}
where \foreachj, $r_{s_j}$ is the rule \gatdisplayrule{Q}{\ofT{s_j}{\Omega_j[\SUBsFORy{j-1}]}}
and $r_{s'_j}$ is the rule \gatdisplayrule{Q}{\ofT{s'_j}{\Omega_j[s_1|y_1,...s_{j-1}|y_{j-1}]}}.

To show that \gatdisplayrule{Q}{\Omega[\SUBsFORy{m}]=\Omega'[s'_1|y_1...s'_m|y_m]} is consistently interpreted by $\Ibar$
we have to show that the rules \ZOmegaSUBsmFORym and \gatdisplayrule{Q}{\isT{\Omega'[s'_1|y_1...s'_m|y_m]}}
...
\ibarmappedrule{Q}{\isT{\Omega[\SUBsFORy{m}]}} 
    ...
    &=\ibarmappedrule{Q}{\isT{\Omega'[s'_1|y_1...s'_m|y_m]}}
