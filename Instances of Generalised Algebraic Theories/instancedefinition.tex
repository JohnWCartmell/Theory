...
\item
If  $Q$ and $\yOmega{m}$ are contexts and if $\tuple{\sm}$ is a realisation of $\yOmega{m}$ wrt $Q$ in $U$
...
where $r_{\Omega_j}$ is the derived rule \IOmega{j}.
\end{itemize}
...
is mapped to   $s(id_{r_{\Omega_m}})$, and
...
$1 \base I(r_{\Delta_1}) \base ... \base I(r_{\Delta_m}) \base I(r)$ in \catc,
...
\highlight{NO SOMETHING WRONG HERE}
\end{proof}
...
then the realisation $\tuple{s_1,...s_m}$ is mapped by $I$ to a cascade within \catc.
\end{lemma}
\begin{proof}
...
$$I(r_{s_m}) \in Sect(I(r_{s_{m-1}})^*...I(r_{s_1})^*(\crossx{I(Q)}{I(r_{\Omega_m})}{1})).$$ \commentary{Tighten this up.}
...
we have that $\tuple{\sm}$ is mapped to a cascade by $I$, by lemma \lref{realisationmapstocascade}. Hence 
...
\end{equation}
and thus that $I(r_{\Delta_n}) \base \Imap{\Qtargetruleone}$ in \catcw as required by clause (a).
...
assume $\tuple{\tn}$ is a realisation of $\xDelta{n}$ with respect to $P$ thsat is mapped to a cascade by $I$, 
we must show that
...
where each $r_{t_i}$ is the rule $\tDelta{i}$.
...
$$           \in Sect\Big(I(r_{t_n})^* ... I(r_{t_1})^*\big(\crossx{I(P)}{\Imappedrule{\xDelta{n}}{\isT{\Omega_j[\SUBsFORy{j-1}]}}} {1}\big)\Big), \mbox{by \highlight{what?}}$$
