...
where $r_{\Omega_j}$ is the derived rule \IOmega{j}. Note that according to this description the empty realisation, which is a realisation of the empty context with respect to any other context, is deemed to map to a cascade under $I$.  \commentary{Do we need modify defn. of cascade so that empty tuple is a cascade?}
\end{enumerate}
...
\end{equation}
and thus\commentary{can we \highlight{reference} a lemma here?} that $I(r_{\Delta_n}) \base \Imap{\targetruleone}$ in \catcw as required by clause (a).
