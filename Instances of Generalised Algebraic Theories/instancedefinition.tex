...
\vspace{2in}
Now suppose that for each $j$, $j < i$, the $\Ibar$ is defined at the rule $r_{x_j}$ and that this rule is consistently interpreted by $\Ibar$, in particular that
$\Ibar(r_{x_j})=s(p_{a_{n_1},a_j})$. We shall now show that it follows that 
...
We have to show that the preconditions for $\Ibar$ to be defined at the rule $r_{x_i}$ hold. These are:

\begin{enumerate}
\item \foreachi, the context $\xDelta{i}$ is mapped by $\Ibar$ to some object $a_i$ of \catcw such
that $1 \base a_1,...\base a_i$, as above, this follows from lemma \lref{Ibartowerlemma}, 
\item the rule \gatdisplayrule{\xDelta{n+1}}{\isT{\Delta_i}} is mapped by $\Ibar$ to the object $\crossx{a_{n+1}}{a_i}{a_{i-1}}$
\end{enumerate}
and we also have to show 
\begin{enumerate}[3.]
...
For substitution into $r_{x_i}$ I am going to need
$\gnvectorstar (\crossx{b}{s(p_{a_n,a_i})}{1})=g_i$ am I not?
...
$\tuple{x_1,...x_i}$ is a realisation of context $\xDelta{i}$ with respect to context $\Delta{n+1}$ 
...
\end{array}
\end{equation*}
since we have assumed that $r_{\Delta_i}$ is consistently interpreted by $\Ibar$ and in particular that clause xxx of that defintion holds
then 
\begin{equation*}
\begin{array}{c c c}
\forceSOURCEwidth & & \forceTARGETwidth \\ [-0.1cm]
...
Except I still need to prove the subtitution property For substitution into $r_{x_i}$ I am going to need
$\gnvectorstar (\crossx{b}{s(p_{a_n,a_i})}{1})=g_i$ am I not? \commentary{Be careful not use substituion before I have proved it} 
