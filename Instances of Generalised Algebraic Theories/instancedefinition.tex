...
$\Ibar(r_{\Delta_i})$ is defined \foreachi, where $r_{Delta_i}$ is the rule \gatdisplayrule{\xDelta{i-1}}{\isT{\Delta_{i}}}.
...
So, suppose that for each $j$, $j < i$, that $\Ibar$ is defined for and consistently interprets the rule $r_{x_j}$. 
...
$\Ibar(r_{x_i})=s(p_{a_{n+1},a_i})$. \commentary{not yet defined $a_i$.}
