...
\newcommand{\SUBsmFORym}{s_1|y_1,...s_m|y_m}
\newcommand{\GRomega}[1][]{\gatdisplayrule[#1]{\yOmega{m}}{\isT{\Omega}}}
\newcommand{\GRsTomega}[1][]    {\gatdisplayrule[#1]{\yOmega{m}}{\ofT{s}{\Omega}}}
\newcommand{\GRomegaSUBsmFORym}[1][]{\gatdisplayrule[#1]{Q}{\isT{\Omega[\SUBsmFORym]}} }
\newcommand{\GRsTomegaSUBsmFORym}[1][]{\gatdisplayrule[#1]{Q}{\ofT{s[\SUBsmFORym]}{\Omega[\SUBsmFORym]}} }
...
Suppose that  the rule $r$ is the T-rule \GRomega and let $r_m$ be the rule
...
\begin{newtt}
if $I$ is defined for each rule \gatdisplayrule{Q}{\sjconclusion} then
\end{newtt}
...
Suppose that $r$ is the $\in$-rule \GRsTomega then
...
Suppose that $r$ is the $\in$-rule \GRsTomega and that
...
\gatdisplayrule{Q}{\sjconclusion} is consistently interpreted by $I$ then
\begin{enumerate}[(i)]
\item if \GRomega is a derived rule which is consistently interpreted by $I$
then the derived rule 
\GRomegaSUBsmFORym is consistently interpreted by $I$,
\item if \GRsTomega is a derived rule which is consistently interpreted by $I$
then the derived rule 
\GRsTomegaSUBsmFORym is consistently interpreted by $I$.
\end{enumerate}
\end{lemma}
\begin{proof}
\highlight{HOPEFULLY}.
...
if \GRomega is a derived of $U$ for which $I$ is defined and which is consistently interpreted by $I$ and if $Q$ is a context which is mapped to an object $a$ by $I$ 
...
Suppose that  the rule $r$ is the T-rule \GRomega and that $r$ is a derived rule of $U$. 
Let $r_j$ be the rule
...
To do this we have to show that the rules \GRomegaSUBsmFORym and \gatdisplayrule{Q}{\isT{\Omega'[s'_1|y_1...s'_m|y_m]}}
...
the first part of what we have to prove, that the rules \GRomegaSUBsmFORym and \gatdisplayrule{Q}{\isT{\Omega'[s'_1|y_1...s'_m|y_m]}}
...
because  from the the fact  that \GRomegaSUBsmFORym is consistently interpreted by $I$ we have that
$$
\Imappedrule{Q}{\isT{\Omega[\SUBsmFORym]}} = \gmstar...\gonestar(\crossx{a}{\Imappedrule{\yOmega{m}}{\isT{\Omega}}}{1})
$$
where \foreachj, $g_j$ is the section
