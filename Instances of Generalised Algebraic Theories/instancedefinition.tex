...
then so to is the rule \gatdisplayrule{\xDelta{n+1}}{\ofT{x_i}{\Delta_{i}}}, \foreachi, where $x_{n+1}$ is any variable distinct from each of the variables $\xn$.
We have to show that if $\Ibar$ is defined for any such rule $r_{\Delta_{n+1}}$ 
and this rule is consistently interpreted by $\Ibar$ then so to is $\Ibar$ defined for each associated rule $r_{x_i}$ and this rulee too is consistently
interpreted by $\Ibar$. We show that this is the case and then show that if it is the case for $r_{x_j}$ all $j <i$ then it follows that is the case for
$r_{x_i}$ also. In this we establish that it holds for all $i \leq n$. 

In the case that $i=1$ we have to show first that the preconditions for $I(r_{x_1})$ hold. These are
that
\begin{enumerate}[(a)]
\item the context $\ofT{x_1}{\Delta_1}$ is mapped by $\Ibar$ to some object $a_1$ of \catc, this follows from the inductive hypothesis \highlight{?},
\item $\Ibar$ maps the rule \gatdisplayrule{\xDelta{n+1}}{\isT{\Delta_{1}}} to the object $???????$, this also follows from the inductive hypothesis \highlight{DOES IT?} from the defintion of $\Ibar$ \highlight{which?}
\end{enumerate} 
