...
iff $I(r_\Omega)$ is defined and $\displaystyle I(r_s) \in Sect(I(r_\Omega))$, where
$r_\Omega$ is the rule \ZOmega, and
either
\begin{enumerate}[(a)]
\item $s$ is the variable $y_j$, \forsomej, so that $r_s$ is the rule \gatdisplayrule{\yOmega{m}}{\ofT{y_j}{\Omega}}
and $I(r_s)$ is the section\footnote{\highlight{CHECK AFTER CHANGE OF CONTEXT}
Implies, by lemma \lref{sofplemma}, that $s(p_{b_m,b_j}): b_m \morph \crossx{b_m}{b_j}{b_{j-1}}$ in \catc.
} % end footnote  
$s(p_{b_m,b_j})$ of the object $\crossx{b_m}{b_j}{b_{j-1}}$ where \foreachj, $b_j$ is the object
$\displaystyle\Imappedrule{\yOmega{j-1}}{\isT{\Omega_j}}$.  \commentary{btw: $$s(id_{b_m})=\delta_{b_m}$$}
Note that $p_{b_m,b_m}$ is defined to be $id_{b_m}$ and so in the case of $j=m$, $r$
is mapped to   $s(id_{b_m})$, or

\item otherwise, $s$ is not simply a variable and 
for all contexts $Q$ and for all realisations $\tuple{\sm}$ of $\yOmega{m}$ wrt $Q$, 
...
\begin{lemma}
\llabel{towerlemma}
\IfIpartialmappingUtoC, suppose that $r$ is the rule \ZDelta and that
...
where, \foreachi, $r_{\Delta_i}$ is the rule \IDelta{i}.
\end{lemma}
\begin{proof}
...
such that \foreachj, $I(r_{s_j})$ is defined 
and $I(r_{s_j}) \in Sect(I(r_{s_{j-1}})^*...I(r_{s_1})^*(\crossx{I(Q)}{I(r_{\Omega_j})}{1}))$,
$$ \Imappedrule{Q}{\ofT{s[\SUBsFORy{m}]}{\Omega[\SUBsFORy{m}]}} = I(r_{s_m})^*...I(r_{s_1})^*(\crossx{I(Q)}{\Imappedrule{\yOmega{m}}{\ofT{s}{\Omega}}}{1}).$$
\end{lemma}
\begin{proof}
The definition of consistent interpretation disrectly requires this for non-variables and so
we only have to show that the definition implies this in the case when $s$ is a variable. Suppose $s$ is the variable
...
$I(r_{\Delta_1}) \base ... I(r_{\Delta_{n-1}}) \base I(Q)$ in \catc.
Also because \foreachj, the rule \IsOmega{j} is consistently interpreted by $I$, 
we have that $I(r_{s_j}) \in Sect(I(r_{s_{j-1}})^*...I(r_{s_1})^*(\crossx{I(Q)}{I(r)}{1})))$, by lemma \lref{substitutionpropertyvariant}. Hence $\Imap{\Qtargetruleone}$ is defined and 
\begin{equation}
\label{omegaSubsmapping}
\Imappedrule{Q}{\isT{\Omega[\SUBsFORy{m}]}} 
= I(r_{s_m})^*...I(r_{s_1})^*(\crossx{a}{I(r)}{1})
\end{equation}
and thus $I(r_Q) \base \Imap{\Qtargetruleone}$ in \catcw as required by clause (a).
...
if \foreachi, $I(r_{t_i})$ is defined and $I(r_{t_i}) \in Sect(I(r_{t_{i-1}})^* ... I(r_{t_1})^*I(P)$ \commentary{$I(P)$ isNot correct}
where $r_{t_i}$ is the rule $\tDelta{i}$ %and where $a_P$ is the object to which $I$ maps context $P$ 
then
...
$$\Imappedrule{\xDelta{n}}{\isT{\Omega[s_1[\SUBtFORx{n}]|y_1,... s_m[\SUBtFORx{n}]|y_m]}}
                =I(r_{t_n})^* ... I(r_{t_1})^*\big(\crossx{I(P)}{I(r_{s_m})^*...I(r_{s_1})^*(\crossx{I(Q)}{I(r)}{1})}{1}\big)$$
...
of the context $Q$ with respect to $P$ and therefore
$$\Imappedrule{P}{\ofT{s_j[\SUBtFORx{n}]}{\IOmegaDoublySubstituted{j}}}
           = I(r_{t_n})^* ... I(r_{t_1})^*\big(\crossx{I(P)}{I(r_{s_j})} {1}\big)$$

\begin{oldtt}
 $\Imappedrule{P}{s_j[\SUBtFORx{n}] \in \IOmegaDoublySubstituted{j}}$ is consistently interpreted by $I$ and in particular
$$\Imappedrule{P}{\ofT{s_j[\SUBtFORx{n}]}{\IOmegaDoublySubstituted{j}}}
            \in I(r_{t_n})^* ... I(r_{t_1})^*\big(\crossx{a_P}{\Imappedrule{\xDelta{n}}{\Omega_j[\SUBsFORy{j-1}]}} {1}\big)$$
which by a $j$'th variant of (\lref{omegaSubsmapping}) is to say that
$$\Imappedrule{P}{\ofT{s_j[\SUBtFORx{n}]}{\IOmegaDoublySubstituted{j}}}
...
\begin{multline}
     \Imappedrule{\xDelta{n}}{\isT{\Omega\big[s_1[\SUBtFORx{n}]|y_1,... s_m[\SUBtFORx{n}]|y_m\big]}} \\
           =  \big(I(r_{t_n})^*...I(r_{t_1})^*(\crossx{I(Q)}{I(r_{s_m})}{1})\big)
               ^* ... 
              \big(I(r_{t_n})^*...I(r_{t_1})^*(\crossx{I(Q)}{I(r_{t_1})}{1})\big)
               ^*\big(\crossx{I(P)}{I(r)}{1}\big)
\end{multline}
This is the equation we are looking for because of the identity

lemma \lref{cascadedpullbackscohere},
...
\begin{newtt}If $I$ is an interpretation of generalised algebraic theory $U$ in contextual catgeory \catcw \highlight{CHECK}\end{newtt}
...
\begin{newtt}and that $\Ibar(big r_{\Delta_i})=\crossx{a_n}{a_i}{1}$ which we have shown in 1. above.\end{newtt} \\
\underline{CF2(a)}
