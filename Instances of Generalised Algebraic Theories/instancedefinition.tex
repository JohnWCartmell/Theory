...
there is at most one such instance determined by $\iI$.
...
I $\iI$ does determine and instance then that instance is $\Ibar$.
...
\IfIpartialmappingUtoC,
if we suppose that $Q$ and $\encyOmega{m}$ are contexts, for some $m \geq 1$, and that $\tuple{\sm}$ is a realisation of $\encyOmega{m}$ wrt $Q$ and
...
\end{lemma}
\begin{proof}
Suppose that $Q$ is the context $\xDelta{n}$, for some $n \geq 0$. Let $r_{\Delta_i}$, \foreachi, be the rule \IDelta{i}.
...
First of all we show  condition (a) that $I(r_{\Delta_n})$ 
and $\Imap{\targetruleone}$ are defined and that 
$I(r_{\Delta_n}) \base \Imap{\targetruleone}$ in \catc. 
First, that $I(r_{\Delta_n})$ is defined follows from the initial assumption
that rule \gatdisplayrule{\xDelta{n}}{\ofT{s_1}{\Omega_1}} is consistently interpreted by $I$
...
of the context $\xDelta{n}$ with respect to $P$ that is mapped to a cascade by $I$ and therefore
...
$$           = Sect\bigg(I(r_{t_n})^* ... I(r_{t_1})^*\Big(\crossx{I(P)}{ I(r_{s_{j-1}})^*...I(r_{s_1})^*\big(\crossx{I(Q)}{I(r_{\Omega_j})}{1}\big)} {1}\Big)\bigg), 
...
$$           =  Sect\bigg(
              ...
              \Big(I(r_{t_n})^*...I(r_{t_1})^*(\crossx{I(r_{\Delta_n})}{I(r_{s_1})}{1})\Big)
               ...
               \bigg) \mbox{by lemma \lref{cascadedpullbackscohere} }
$$
and so 
...
Now let $r_{\Omega_s}$ be the rule \ZsOmega and assume this rule to be consistently interpreted by $I$ as defined
in clause (ii) of definition \lref{consistentinterpretation}. Condition (b) of the defitnion tells us that
\targetruleone must be consistently interpreted by $I$ and therefore in this part (ii) of the proof we can use
anything established in part (i) of the proof.
...
which follows from the initial assumption that \ZsOmega is consistently interpreted by $I$ 
and because we have already established that $\tuple{\sm}$ is a realisation which is mapped to a cascade by $I$
and from which we also deduce that
\begin{equation}
...
this follows because $$\tuple{s[s_1[\SUBtFORx{n}]|y_1,... s_m[\SUBtFORx{n}]|y_m]}{\Omega[s_1[\SUBtFORx{n}]|y_1,... s_m[\SUBtFORx{n}]|y_m]}$$
is a realisatiion which is mapped by $I$ to a cascade,
and, in this case, because of the initial assumption that 
\ZsOmega is a derived rule which is consistently interpreted by $I$
and use of equation (\lref{cascadedpullbackscohereonsections}) of lemma \ref{cascadedpullbackscohere}.
...
so that the expression $s[\SUBsFORy{m}]$ is simply the variable $x_i$ and so that to show condition (d) we need to show that
...
mapping $\Ibar$  is total i.e. is defined for all derived T- and $\in$-rules and the instance determined by $\iI$ is $\Ibar$.
\end{lemma}
\begin{proof}  
