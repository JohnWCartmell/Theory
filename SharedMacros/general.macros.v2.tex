...
% WHAT I SHOULD DO
% create a spearate macros file with the following content:


\usepackage{imakeidx} % make sure this package is loaded

% Declare the indexes used by macros
%\makeindex[name=lemmas,title=Lemma Index]

% Commentary macro (prints in margin)
%\newcommand{\commentary}[1]{\marginpar{\footnotesize #1}}

% llabel: label + commentary + indexing
%\newcommand{\llabel}[1]{%
%  \label{#1}%
%  \commentary{\colorbox{pink}{\scriptsize #1}}%
%  \index[lemmas]{#1}%
%}

%%%% definition of pink if needed.

% lref: reference + pink box + index
%\newcommand{\lref}[1]{%
%  \ref{#1}%
%  \colorbox{pink}{\scriptsize{#1}}%
%  \index[lemmas]{#1!use of}%
%}
...
% after moving to textlive... dont have mnsymbol therefore
\newcommand{\rightpitchfork}{\mathrel{\raisebox{-0.0375cm}{\rotatebox[origin=c]{270}{$\pitchfork$}}}} %in place of mnsymbol
...
\newcommand{\redlight}[1]{\colorbox{red}{#1}}
\newcommand{\term}[1]{\textit{#1}\commentary{\colorbox{lightgray}{\textit{#1}}}\index[definitions]{#1}}
\definecolor{defcolor}{RGB}{255,230,230}   % very light pink
\definecolor{refcolor}{RGB}{230,240,255}   % very light blue
%\newcommand{\llabel}[1]{\label{#1}\commentary{\colorbox{defcolor}{\scriptsize{#1}}}\index[lemmas]{#1}}
...
\newcommand{\lref}[1]{\ref{#1}\colorbox{refcolor}{(\scriptsize #1)}\index[lemmas]{#1!use of}}
...
% Text that I am working on
\newcommand{\workt}[1]{\colorbox{Apricot}{#1}}
\newenvironment{worktt}
{  \colorbox{Apricot}{$[$ ...} 
}
{  \colorbox{Apricot}{... $]$}
}
...
\newenvironment{showexample}
  {\VerbatimOut{temp.tmp}}
  {\endVerbatimOut
   \VerbatimInput{temp.tmp}
   \medskip
   {\begingroup\input{temp.tmp}\endgroup}
   \hrule}
