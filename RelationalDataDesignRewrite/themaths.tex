...
for every node $a$ other than $\veee$ there is a non-empty set  of identifying edges with domain $a$.
 We denote by $I_a$ this non-empty set of identifying edges leaving an entity type $a$.
}
\end{enumerate}
...
\end{definition}
We say that a referential inclusion dependency in model \genericmodel\ is represented provided that
...
$\qntuple \in \bar{I}_d$ \commentary{Need to CHECK THIS OUT AFTER REWORD}
...
In a well-founded model \genericmodel\ if $Q$ is a identifying set of paths\commentary{WHAT IS ALL THIS ABOUT?} from entity type $a$ to $\veee$
...
\commentary{Want to move this to end but currently this is used in main theorem.}
\begin{definition}
...
\item
for each relationship $r$ there is there is an inclusion dependency that it represents.\commentary{Dont think this is enough}.
\end{enumerate} 
\end{definition}
...
E_{\langle x_1,...x_n \rangle}\circ E^{-1}_{\qntuple} \circ E_{z} = E_{\langle y_1,..y_m \rangle} 
\end{equation}
...
\langle E_{x_1},...E_{x_n} \rangle \circ E^{-1}_{\qntuple} = E_{\langle p_1,...p_k \rangle}
\end{equation}
