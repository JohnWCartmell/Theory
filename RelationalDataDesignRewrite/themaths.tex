...
\end{figure}
\commentary{Relational meta-schema path equivalence example is a little inaccessible therefore consider replcing to reference to figure 3 which is the table row column data cell example. actually called example of path euivalence currently.}
...
\begin{definition}
Say that two models are \term{co-extensional} \commentary{was \term{equivalent}} iff there is a third model that they are both extensions.
\commentary{Co-extsionnality is not a transitive and so we need equivalence needs be the symetric transitive closure.
But do I need this closure? And is this closure the right thing categoricaly?}
\end{definition}
...
\commentary{$\bar{I}$ has not been defined.}
