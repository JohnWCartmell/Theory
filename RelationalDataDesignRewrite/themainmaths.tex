...
\begin{lemma}
If $\gmodel$ is an ER model then it can be extended to a model $\gmodel'$
which is primarily keyed. If $\gmodel$ satisfies the minimality condition then the
extended model $\gmodel'$ satisfies the  condition that each set $I_a$ has no proper subsets which are mono-sources. 
\end{lemma}
\begin{proof}
...
finite set of attributes along with a set of semantic constraints. The latter may include functional dependencies or keys and key attributes and inclusion depedencies. If  an n-ary relation $R$ has attributes
$X_1,X_2...X_n$ then we shall write it as $R(X_1,...X_n)$. We restrict our attention, for the time being, to those schemas in which for each relation there is a subset of its attributes that comprise a primary key and between these relations there are defined  a set of inclusion dependencies of the 
...
If $K$ is the primary key we shall say that such an inclusion dependency is a \term{primary keyed inclusion dependency}. If $R$ is a relation having primary key consisting
...
The following definition uses the definitions of \term{primary referential inclusion dependency} and 
\term{simple referential inclusion dependency} that were given in 
section \ref{referentialinclusiondependency}.
\begin{definition}
\noindent 
...
\item
for each relationship $r$ there is a simple primary referential inclusion dependency that it represents,
\item 
for each simple primary referential inclusion dependency there is a unique relationship which represents it.
\end{enumerate} 
\end{definition}
...
\item a keyed inclusion dependency is defined to correspond to every simple primary referential inclusion dependency of $\gmodel$ i.e. to correspond to every relationship of $\gmodel$. 
\end{itemize}
...
Likewise every relational schema that consists of relations with primary keys and primary keyed inclusion dependencies and has a set of characteristic database instances gives rise to an equivalent primary relational ER model. 

For the time being we restrict attention to prime relational ER models which stand-in for us for a large class of relational schemas. 
