...
referential attributes can be extended to one that does by the addition of certain
edges and that  the original and the extended model will be equivalent in the way described by the definition of equivalence that we can now give.  

\begin{definition}
A ER model mapping $\varphi: \gmodel \morph \gmodel'$ is said to be an equivalence
and the models $\gmodel$ and $\gmodel'$ are said to be equivalent iff there is a ER model mapping $\psi: \gmodel' \morph \gmodel$ such that
\begin{enumerate}[(i)]
...
If instead a relationship $r$ directly depends one or more paths of $\gmodel$ then define its rank to be 1 greater than the maximum rank of the relationships it depends on. 
...
\begin{cases} 
    ...
    s_i         & \mbox{if $s_i$ is an attribute-like path and is equivalent to  $r \circ q_i$ in $\gmodel$ }  \\
...
if $p:b \morph \veee$ is a path in $\chigmodel$ then there exists a path $\retract(p):b \morph \veee$ in $\gmodel$ such that $I_\gmodel(p') \simeq p$ in $\chigmodel$. Finally note that if $\retract(p)$ is a primary key path in $\gmodel$ then there is an attribute $k$ which is a primary key path
...
\begin{lemma}
\label{fdcarrythroughsublemma}
If $\gmodel$ is a $\chit$-suitable ER model and if $\xn$ and $y$ are paths in $\chigmodel$
such that there is a functional dependency
\begin{equation}
\label{targetfd}
\fd{\xn}{y}
\end{equation}
...
\begin{equation}
\label{sourcefd}
\fd{\retract(x_1),...\retract(x_n)}{\retract(y)}
\end{equation}
in $\gmodel$. Further,  (\ref{targetfd}) is  leaf  iff (\ref{sourcefd}) is a leaf  and (\ref{targetfd}) is  intransitive iff (\ref{sourcefd}) is a intransitive.

If a path $p$ represents (\ref{sourcefd}) in $\gmodel$ then $\inject(p)$ represents (\ref{targetfd})
in $\chigmodel$.
\end{lemma}
\begin{proof}
TBD
...
\begin{lemma}
\label{incdcarrythroughsublemma}
If $\gmodel$ is a $\chit$-suitable ER model and if $\xn$ and $\qn$ are paths in $\chigmodel$
such that there is an inclusion dependency
\begin{equation}
\label{targetincd}
\incd{a}{\xn}{b}{\qn}
\end{equation}
 in $\chigmodel$ 
then  there is a inclusion dependency
\begin{equation}
\label{sourceincd}
\incd{a}{\retract(x_1),...\retract(x_n)}{b}{\retract(q_1),...\retract(q_n)}
\end{equation}
in $\gmodel$. 

Further,  (\ref{targetfd}) is  ???  iff (\ref{sourcefd}) is a ???  and (\ref{targetfd}) is  ??? iff (\ref{sourcefd}) is a ???.

If a path $p$ represents (\ref{sourceincd}) in $\gmodel$ then $\inject(p)$ represents (\ref{targetincd})
in $\chigmodel$.
\end{lemma}
\begin{proof}
TBD
...
\end{lemma}
\begin{proof}
Use lemma \ref{fdcarrythroughsublemma}.
\vspace{0.5cm}
tbd
\end{proof}
...
If  $\gmodel$ is $\chit$-suitable and has the $\fdfactoring$ property then so to does $\chigmodel$. 
\end{lemma}
\begin{proof}
Use lemma \ref{fdcarrythroughsublemma}.
\vspace{0.5cm}
...
\end{corollary}
\begin{proof}
Follows directly from theorems \ref{maintheorem} and \ref{chitheorem}.
\end{proof}
