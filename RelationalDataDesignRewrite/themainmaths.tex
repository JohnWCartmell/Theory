...
\item
for each relationship $r$ there is  a referential inclusion dependency that it represents,
\item 
for each minimal referential inclusion dependency there is a unique relationship which represents it.
\end{enumerate} 
\end{definition}
\begin{remark}
...
of a transformation first described by Chen and subsequently described in a range of software design methodologies and implemented in associated CASE tools. This earlier transformation is sometimes said to produce a first-cut relational design because generally the model produced requires normalisation
to achieve an appropriate database normal form such as 3NF or BCNF. 
We shall show that with these refinements 
the transformation $\chi$ produces relational models that are in 3NF and, if appropriate, BCNF.
...
The final refinement is in recognition that certain attribute-like paths may be equivalents of one another.
...
a path which renders them redundant. If we denote the set of equivalence classes of $\attr[\chiOneM]{a}$
