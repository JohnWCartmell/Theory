...
\item
for each relationship $r$ there is a referential inclusion dependency not using $r$ \commentary{Need define \term{not using}.Rememeber $r$ might have equivalent path I think. PROBLEM: CIRCULARITY}that it represents,
\vspace{1cm}
\item 
for each minimal [intransitive?] \commentary{Think about this.} referential inclusion dependency there is a unique relationship which represents it.
\end{enumerate} 
\end{definition}
\begin{remark}
...
In recognition that certain attribute-like paths may be equivalents of one another. if two edges are equivalent then omit one of them.
