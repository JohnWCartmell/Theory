...
\begin{definition}
Say that two models are \term{co-extensional} \commentary{was \term{equivalent}} iff there is a third model that they are bo and such thatth extensions of.
...
and a  path $z:d \morph c$  such that $y \simeq p \circ z$. If $c$ is $\veee$ then
the path $z$ is not equivalent to an identifying path.
\end{lemma}
\begin{proof}
...
path $z:d\rightarrow c$, for some $l \geq 0$ as here:
