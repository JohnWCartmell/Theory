...
relationship $w: b \morph c$ so that there is a functional dependency $\sfd{x}{y}$ in $\genericmodel$
...
Generally a relational schema is defined to consist of (i) a finite set of relations each consisting of a
finite set of attributes (ii) a set of semantic constraints. The latter include functional dependencies, usually expressed as in terms of keys and key attributes. If  an n-ary relation $R$ has attributes
...
\subsection{Definition of Relational ER Model}
\begin{definition}
\noindent 
A \term{relational ER model} is an  ER model that also satisfies:
\begin{enumerate}[(i)]
\item
...
and simple primary referential inclusion dependencies.
\end{remark}
...
it follows that $y$ is rendered redundant by $p \circ w$ in $\genericmodel$, 
