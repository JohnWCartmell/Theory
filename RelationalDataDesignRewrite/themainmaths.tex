...
\begin{definition}
An ER model $\gmodel$ is \term{restriction-free} iff for all entity types $a$ of $\gmodel$, and for all paths $x_1,...x_n$ and $y$  with domain
$a$ in $\gmodel$,  if for some $j$, $1 \leq j \leq n$, in every defining instance $E$,  
...
In a well-formed model $\gmodel$ that is restriction-free, if $\set{x_1,...x_n} \morph y$  is a non-trivial intransitive leaf
...
$i$, $1 \leq i \leq n$.  Because $p$ represents the inclusion dependency we know  that $p \circ q_i \leq x_i$. 
Hence $y \leq x_i$. See A4 page.  Hence $\gmodel$ is restriction-free then $y \simeq x_i$. This contradicts the assumption that the functional dependency
is non-trivial.
\end{proof}
...
\item the attribute $x$ is identifying, or
...
\begin{definition} 
A relational ER model $\gmodel$ is \term{simple without caveat} iff for all attributes $x:a \morph \veee$
such that there is a relationship-like path $r:a \morph b$ and a path $p:b \morph \veee$ such that
$x \simeq r \circ p$ then there exists an identifying attribute $q:b \morph \veee$ such that $p \simeq q$.
\end{definition}
...
\begin{theorem}
\noindent If a relational ER model $\gmodel$ is well-formulated, has the \fdfactoring\ property and is reduced then it is in Strong Third Normal Form. 
...
If the model $\gmodel$ is assumed to be reduced without caveat then either $r$ is a mono-source or $w$ is equivalent to some primary key of $d$.
If $r$ is a mono-source then $x_1,...x_n$ is a mono source, as required, since $\sfd{x_1,...x_n}{r}$.
If $w$ is equivalent to some primary key of $d$ then it is equivalent to $q_i$, for some $i$, $1 \leq i \leq n$, therefore 
$E_y =  r \circ w = r \circ q_i = x_i$ and the functional dependency $\sfd{x_1,...x_n}{y}$ is trivial, contradicting our initial assumption.
If the model $\gmodel$ is reduced without caveat then there are no other cases and we have established BCNF.
If not, with the weaker assumption that the model $\gmodel$ is reduced, there is the possibility that $y$ is identifying i.e. is a primary key attribute and we have proved S3NF.
\end{proof}
...
For the transformation we describe to produce a relational model we require the model $\gmodel$ to have 
property X defined as follows:
\begin{definition}
An ER model $\gmodel$ is said to have property X iff for all relationships $r: a \morph b$ in $\gmodel$ and for all
primary mono-sources $\set{q_1,...q_n}$ with domain $b$, there exists $i$, $1 \leq i leq n$, such that $r \circ q_i$
is not dominated i.e. such that there does not exist a path $s: s \morph v$ such that $r \circ q_i < s$.  
\end{definition}
...
is not attribute-like, \\
 \hspace{1cm} or:
\item $n=1$ and $r_1 \simeq \tuple{s_0,s_1,...s_m}$ and $r_1$ is not identifying and $\tuple{s0,s_1,...s_m}$ is not attribute-like.
...
$\tuple{t_1,t_2...t_{l}} \simeq \tuple{r_1,...r_n}$ in $\gmodel$
 there exists a \commentary{attribute-like path??} path $\tuple{t_1,t'_2...t'_{l'}}$  in $\gmodel$ which is not dominated.
\end{enumerate}
\end{definition}
...
and property X then the transformed model $\genericphysical$ is a well-formulated, restriction-free relational ER model with the fd factoring property which is therefore in S3NF.  Further we will show that if $\gmodel$ is without caveats then $\genericphysical$ is also without caveats and is therefore in BCNF. 

\begin{lemma}
If $\gmodel$ has property X and is tangle-free then $\genericphysical$ is relational.
\end{lemma}
