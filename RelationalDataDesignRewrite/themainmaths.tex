...
have codomain $\veee$.
\end{definition}
To reach the goal of showing that to every ER model there is an equivalent relational model we require the following\commentary{is this true?}:
\begin{lemma}
...
\begin{lemma}
\label{fdcarrythroughone}
If $\gmodel$ and $\gmodel'$ are ER models and  if  $\varphi$ is an ER model equivalence between $\gmodel$  and $\gmodel'$,  if $\xn$ and $y$ are paths in $\gmodel$
...
\end{equation}
in $\gmodel'$.
Further,  (\ref{targetfd}) is  leaf  iff (\ref{sourcefd}) is a leaf  and (\ref{targetfd}) is  intransitive iff (\ref{sourcefd}) is a intransitive.
...
If two models are equivalent then one has the fd factoring property ifff the other one does.
...
\commentary{When emphasis is shifted to equivalent models then this lemma should be obvious as to become not worth stating.}
\end{lemma}
