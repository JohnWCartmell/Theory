...
\begin{theorem}
\noindent If a relational ER model $\gmodel$ is well-formulated, has the \fdfactoring\ property and is dense then it is in Strong Third Normal Form. 
...
If the model $\gmodel$ is assumed to be strongly dense then either $r$ is a mono-source or $w$ is equivalent to some primary key of $d$.
...
If not, with the weaker assumption that the model $\gmodel$ is dense, there is the possibility that $y$ is identifying i.e. is a primary key attribute and we have proved S3NF.
\end{proof}
