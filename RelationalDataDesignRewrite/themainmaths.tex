...
\begin{definition}
\noindent An ER model $\genericmodel$ has the \term{\fdfactoring} property iff to every functional dependency $\set{x_1,...x_n} \morph z$ where $x1, x_2, ... x_n$ and $z$ are attribute-like paths either $\set{x_1,...x_n}$ is
...
\section{Relational Models}\commentary{relational?}
...
A relational ER model is in Third Normal Form (TNF)  iff
for all entity types $a$, for all attributes $x_1,...x_n$ and $y$ of $a$, for $n \geq 1$, 
for which  there is a non-trivial functional dependency \sfd{x_1,...x_n}{y}, 
...
for all entity types $a$, for all attributes $x_1,...x_n$ and $y$ of $a$, for $n \geq 1$, 
for which there is an non trivial  functional dependency \sfd{x_1,...x_n}{y},
...
\begin{lemma}
\label{S3NFsublemma}
A model $\genericmodel$ is in S3NF iff
for all entity types $a$, for all attributes $x_1,...x_n$ and $y$ of $a$, for $n \geq 1$, 
for which there is an non trivial  functional dependency \sfd{x_1,...x_n}{y},
...
it follows that $p \circ z $ subsumes $y$ in $\genericmodel$, 
...
Therefore from the defintion of subsumes ,  $y$ is identifying as required to establishing S3NF
