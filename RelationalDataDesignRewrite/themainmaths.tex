...
or else for some entity type $d$ there is a non-empty path $p:a \rightarrow d$ such that $\{x_1,...x_n\} \morph p$\commentary{Added $\{x_1,...x_n\} \morph p$}
...
or else there is an entity type $d$ and there is a path $p:a \rightarrow d$ such that $\{x_1,...x_n\} \morph p$\commentary{Added $\{x_1,...x_n\} \morph p$}
and a  path $w:d \morph c$  such that $y \simeq p \circ w$. If $c$ is $\veee$ then
...
\end{definition}
\begin{definition}
Say that a relational ER model $\gmodel$ is \term{free of attribute redundancy without caveats} iff 
it is free of attribute redundacy and ... \commentary{what goes here?}
\end{definition}
...
\begin{definition} 
A relational ER model is \term{dense} iff for all attributes $x:a \morph \veee$
such that there is a relation $R:a \morph b$ and a path $p:b \morph \veee$ such that
$x \simeq R \circ p$ it follows that 
either 
\begin{enumerate} [(i)]
\item there exists an identifying attribute $q:b \morph \veee$ such that $p \simeq q$, or
\item the relationship $R$ is a mono-source, or
\item the attribute $x$ is identifying.
\end{enumerate}
\end{definition}

We get a stronger condition if we drop condition (iii) in the above definition to get the following:

\begin{definition} 
A relational ER model is \term{strongly dense} iff for all attributes $x:a \morph \veee$
such that there is a relation $R:a \morph b$ and a path $p:b \morph \veee$ such that
$x \simeq R \circ p$ it follows that 
either 
\begin{enumerate} [(i)]
\item there exists an identifying attribute $q:b \morph \veee$ such that $p \simeq q$, or
\item the relationship $R$ is a mono-source.
\end{enumerate}
\end{definition}

\noindent Now for the main theorem:\\
\begin{theorem}
\noindent If an relation ER model $\gmodel$ is well-formulated, has the \fdfactoring\ property and is dense then it is in Strong Third Normal Form. 
If  $\gmodel$ is strongly dense then is in Boyce-Codd Normal form.
\end{theorem}
\begin{proof}
...
or else there is a non-empty path $p$ in $\gmodel$
...
Now because $p$ represents an inclusion dependency then get to a relationship $R$ in place of $p$:
...
Because the model $\gmodel$ is assumed to be dense then either $R$ is a mono-source or $w$ is equivalent to some primary key of $d$
 or $y$ is identifying. If $R$ is a mono-source then $x_1,...x_n$ is a mono source, as required, since $\sfd{x_1,...x_n}{R}$.
...
If the model $\gmodel$ is strongly dense then there are no other cases and we have established BCNF.
If not there is the possibility that $y$ is identifying i.e. is a primary key attribute and we have proved S3NF.
\end{proof}
